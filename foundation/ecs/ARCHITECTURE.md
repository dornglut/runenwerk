...
Policy for structural mutation in Phase 3:
...
- required-present component requirements (query components + `With<T>`/filter requirements)
- excluded component filters from `F`
- access metadata
- per-query `last_run_tick`

Query matching path:

- candidate iteration starts from the smallest required-present component store when possible
- candidate entities are then validated against remaining required/excluded constraints
- `QueryState::get` now performs direct per-entity constraint checks instead of rebuilding full match sets
...
- per-component/per-entity last-changed tick maps back `Changed<T>` checks
- per-component/per-entity last-added tick maps back `Added<T>` checks
- removed components clear per-entity tick entries for that component type
