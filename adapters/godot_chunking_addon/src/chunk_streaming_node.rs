use chunking::{ChunkLoadOrder, ChunkStreamer, ChunkStreamingConfig, ChunkStreamingMode, StreamingFocus};
...
impl ChunkStreamingNode {
	...
	pub fn update_focus_from_vector3(&mut self, position: Vector3) {
		...
		for chunk in entered {
			self.signals().chunk_entered().emit(chunk.x, chunk.y, chunk.z);
		}

		for chunk in exited {
			self.signals().chunk_exited().emit(chunk.x, chunk.y, chunk.z);
		}
