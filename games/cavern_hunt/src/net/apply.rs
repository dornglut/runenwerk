    ...
    for message in events {
        match message {
            ServerMessage::RunEvent(run_event) => {
                if matches!(
                    CavernRunEventCodeV2::parse(run_event.code.as_str()),
                    Some(CavernRunEventCodeV2::Chunk)
                ) {
                    metrics.bytes_received_last_frame = metrics
                        .bytes_received_last_frame
                        .saturating_add(run_event.payload.len() as u64);
                    let chunk: CavernRunEventChunkV2 = postcard::from_bytes(&run_event.payload)?;
                    if world.resource::<ClientRunEventChunkStateV2>().is_err() {
                        world.insert_resource(ClientRunEventChunkStateV2::default());
                    }
                    let reassembled = {
                        let mut state = world.resource_mut::<ClientRunEventChunkStateV2>()?;
...
fn route_run_event(
    ...
    geometry_events: &mut Vec<snapshot::CavernGeometryEditsEventV2>,
    keyframes_by_stream_cursor: &mut BTreeMap<u64, CavernKeyframeEventV2>,
    patches_by_stream_cursor: &mut BTreeMap<u64, CavernPatchEventV2>,
) -> Result<()> {
    ...
    match CavernRunEventCodeV2::parse(run_event.code.as_str()) {
        Some(CavernRunEventCodeV2::GeometryEdits) => {
            let event: snapshot::CavernGeometryEditsEventV2 =
                postcard::from_bytes(&run_event.payload)?;
            geometry_events.push(event);
        }
        Some(CavernRunEventCodeV2::Keyframe) => {
        ...
        Some(CavernRunEventCodeV2::Patch) => {
        ...
        Some(CavernRunEventCodeV2::Chunk) | None => {}
    }
    Ok(())
}
