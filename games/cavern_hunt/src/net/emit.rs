...
use engine_net::{ServerMessage, ServerSessionState};
use serde::{Deserialize, Serialize};

use super::chunking::{chunk_group_id_for_event, split_run_event_for_mtu};
...
struct CavernGeometryEditsEventV2 {
    tick: SimulationTick,
    from_index: usize,
    to_index: usize,
...
pub(super) fn server_emit_replication_v2(world: &mut World) -> Result<()> {
    ...
    let force_bootstrap_keyframe = {
        let state = world.resource::<ServerReplicationStateByConnection>()?;
        state
            .cursors_by_connection
            .values()
            .any(|cursor| cursor.stream_cursor == 0)
    };
    ...
    let geometry_event_payload = {
        ...
        if to_index > from_index {
            ...
            let event = CavernGeometryEditsEventV2 {
                tick,
                from_index,
                to_index,
        ...
        && let Ok(mut outbox) = world.resource_mut::<NetworkServerOutbox>()
    {
        ...
        let event_code = CavernRunEventCodeV2::GeometryEdits;
        let chunk_group_id = chunk_group_id_for_event(tick, 0, event_code.as_str());
        for run_event in split_run_event_for_mtu(event_code.as_str(), payload, chunk_group_id)? {
            outbox.push(ServerMessage::RunEvent(run_event));
        }
    }
    ...
    let mut replication_map = world
        ...
        .cloned()
        .unwrap_or_default();
    let (event_code, payload_len, payload, patch_stats, stream_cursor) = {
        ...
        let emit_keyframe =
            force_bootstrap_keyframe || stream_cursor == 1 || tick.0 % keyframe_interval == 0;
    ...
    if let Ok(mut outbox) = world.resource_mut::<NetworkServerOutbox>() {
        let chunk_group_id = chunk_group_id_for_event(tick, stream_cursor, event_code.as_str());
        for run_event in split_run_event_for_mtu(event_code.as_str(), payload, chunk_group_id)? {
            outbox.push(ServerMessage::RunEvent(run_event));
        }
    }
