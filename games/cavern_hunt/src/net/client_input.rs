...
use engine_net::{ClientMessage, InputFrame};

use crate::{CavernControlState, CavernPredictedFrame, CavernPredictionState};
use crate::net::{AbilityCommand, AimCommand, CavernCommandEnvelope, InteractCommand, MoveCommand};
...
pub(super) fn client_send_control_input(world: &mut World) -> Result<()> {
    ...
    if let Ok(mut outbox) = world.resource_mut::<NetworkClientOutbox>() {
        ...
        let payload = postcard::to_allocvec(&commands)?;
