...
#[deprecated(note = "use ReplicationDriver + SnapshotApplyDriver + InputDriver")]
pub trait LegacyReplicationDriver {
...
impl<TLegacy> ReplicationDriver for TLegacy
where
    TLegacy: LegacyReplicationDriver,
{
    type Snapshot = TLegacy::Snapshot;
    type Delta = TLegacy::Delta;
    type Input = TLegacy::Input;
    type Error = TLegacy::Error;

    fn capture_snapshot(world: &World) -> Result<Option<Self::Snapshot>, Self::Error> {
        TLegacy::capture_snapshot(world)
    }

    fn build_delta(previous: &Self::Snapshot, current: &Self::Snapshot) -> Self::Delta {
        TLegacy::build_delta(previous, current)
    }

    fn apply_delta_to_snapshot(base: &Self::Snapshot, delta: &Self::Delta) -> Self::Snapshot {
        TLegacy::apply_delta_to_snapshot(base, delta)
    }

    fn encode_input(input: &[Self::Input]) -> Result<Vec<u8>, Self::Error> {
        TLegacy::encode_input(input)
    }

    fn decode_input(bytes: &[u8]) -> Result<Vec<Self::Input>, Self::Error> {
        TLegacy::decode_input(bytes)
    }

    fn encode_snapshot(snapshot: &Self::Snapshot) -> Result<Vec<u8>, Self::Error> {
        TLegacy::encode_snapshot(snapshot)
    }

    fn decode_snapshot(bytes: &[u8]) -> Result<Self::Snapshot, Self::Error> {
        TLegacy::decode_snapshot(bytes)
    }

    fn encode_delta(delta: &Self::Delta) -> Result<Vec<u8>, Self::Error> {
        TLegacy::encode_delta(delta)
    }

    fn decode_delta(bytes: &[u8]) -> Result<Self::Delta, Self::Error> {
        TLegacy::decode_delta(bytes)
    }

    fn map_codec_error(error: postcard::Error) -> Self::Error {
        TLegacy::map_codec_error(error)
    }
}

impl<TLegacy> SnapshotApplyDriver for TLegacy
where
    TLegacy: LegacyReplicationDriver,
{
    fn apply_snapshot(
        ...
        TLegacy::apply_snapshot(world, tick, snapshot)
    }
    ...
    fn apply_delta(
        ...
        TLegacy::apply_delta(world, tick, delta)
    }
}

impl<TLegacy> InputDriver for TLegacy
where
    TLegacy: LegacyReplicationDriver,
{
    fn receive_remote_input(
        ...
        TLegacy::receive_remote_input(world, tick, input)
    }

    fn take_local_input(world: &mut World) -> Result<Vec<Self::Input>, Self::Error> {
        TLegacy::take_local_input(world)
    }

    fn apply_input(world: &mut World, input: &[Self::Input]) -> Result<(), Self::Error> {
        TLegacy::apply_input(world, input)
    }
}
