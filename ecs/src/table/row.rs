...
impl<'a> Row<'a> {
	...
	pub fn set<T: 'static>(&mut self, column_index: usize, value: T) -> Result<(), RowError> {
		let column = self
			...
			.ok_or(RowError::InvalidColumnIndex { index: column_index })?;

		let typed_col = column
			.as_any_mut()
			.downcast_mut::<Column<T>>()
			.ok_or(RowError::TypeMismatch { index: column_index })?;
	...
	pub fn get<T: 'static>(&self, column_index: usize) -> Result<&T, RowError> {
		...
		typed_col
			.get(self.row_index)
			.ok_or(RowError::SwapRemoveOutOfBounds { index: column_index })
	}
	...
	pub fn swap_remove_column(
		&mut self,
		column_index: usize,
	) -> Result<Box<dyn Any>, RowError> {
		let column = self
			.columns
			.get_mut(column_index)
			.ok_or(RowError::InvalidColumnIndex { index: column_index })?;
	...
	pub fn swap_remove_row(&mut self) -> Result<(EntityHandle, Vec<Box<dyn Any>>), RowError> {
		...
		let entity = self.entity_column
			.swap_remove(self.row_index)
			.ok_or(RowError::SwapRemoveOutOfBounds { index: self.row_index })?;
