...
use crate::{AnyStorage, ArchetypeKey, Column, ComponentKey, ComponentRegistry, EntityHandle, Row};
...
impl Archetype {
    pub fn new(
        ...
        let types_sorted = component_keys.iter().map(|c| c.type_id).collect::<Vec<_>>();
        assert_eq!(
            types_sorted.len(),
            component_keys.len(),
    ...
    pub fn add_row(&mut self, entity: EntityHandle, mut components: HashMap<TypeId, Box<dyn Any>>) {
        assert_eq!(
            components.len(),
            self.columns.len(),
    ...
    pub fn columns2_mut_immut<A: 'static, B: 'static>(
        ...
        if mut_idx < read_idx {
            ...
            let read_col = right[0].as_ref().as_any().downcast_ref::<Column<B>>()?;
            ...
            let mut_col = right[0].as_mut().as_any_mut().downcast_mut::<Column<A>>()?;
            Some((mut_col, read_col))
        }
    }

    pub fn new_from_registry(keys: &[ComponentKey], registry: &ComponentRegistry) -> Self {
        let constructors: HashMap<_, _> = keys
            .iter()
            .map(|k| {
                (
                    k.type_id,
                    *registry.get_constructor(k).expect("Unregistered"),
                )
            })
            .collect();
    ...
    pub fn validate(&self) -> bool {
        self.columns
            .iter()
            .all(|c| c.len() == self.entity_column.len())
    }
