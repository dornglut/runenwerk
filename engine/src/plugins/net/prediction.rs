...
impl ClientPredictionIntegration {
    fn new(lineage: runen_net::replication::ReplicationLineageKey, policy: ClientPredictionPolicy) -> Self {
        Self { semantic: PredictionLineage::new(lineage, policy.limits()) }
    }
    pub(crate) fn require_connection_replacement<S>(&mut self, replication: &mut ClientReplicationSet<S>) -> Result<(), runen_net::replication::ClientSetError> {
        self.semantic.require_connection_replacement_full(replication)
    }
}
pub(crate) fn configure_client_prediction(app: &mut crate::App, prediction_policy: Option<ClientPredictionPolicy>, replication_policy: Option<ClientReplicationPolicy>) {
    let Some(prediction_policy) = prediction_policy else { return; };
    let replication_policy = replication_policy.expect("tracked client prediction requires explicit ClientReplicationPolicy");
    app.insert_resource(ClientPredictionIntegration::new(replication_policy.lineage(), prediction_policy));
}
pub fn client_prediction_state(world: &World) -> Option<RunenNetPredictionState> {
    world.resource::<ClientPredictionIntegration>().ok().map(|i| i.semantic.state())
}
pub fn client_prediction_pending_count(world: &World) -> Option<usize> {
    world.resource::<ClientPredictionIntegration>().ok().map(|i| i.semantic.pending_count())
}
pub fn client_prediction_pending_bytes(world: &World) -> Option<usize> {
    world.resource::<ClientPredictionIntegration>().ok().map(|i| i.semantic.pending_bytes())
}
pub fn client_prediction_connection_lost(world: &mut World) -> anyhow::Result<()> {
    world.resource_mut::<ClientPredictionIntegration>().context("client prediction requires explicit ClientPredictionPolicy")?.semantic.connection_lost(); Ok(())
}
pub fn client_prediction_participant_membership_ended(world: &mut World) -> anyhow::Result<()> {
    world.resource_mut::<ClientPredictionIntegration>().context("client prediction requires explicit ClientPredictionPolicy")?.semantic.participant_membership_ended(); Ok(())
}
pub fn client_prediction_session_closed(world: &mut World) -> anyhow::Result<()> {
    world.resource_mut::<ClientPredictionIntegration>().context("client prediction requires explicit ClientPredictionPolicy")?.semantic.session_closed(); Ok(())
}
fn admit_client_prediction(world: &mut World, tick: SimulationTick, payload: &[u8]) -> Option<PredictionInputOutcome> {
    world.resource_mut::<ClientPredictionIntegration>().ok().map(|i| i.semantic.admit_local(RunenNetSimulationTick::new(tick.0), &payload.to_vec(), payload.len()))
}
pub(crate) fn mark_client_prediction_local_application_failed(world: &mut World) {
    if let Ok(i)=world.resource_mut::<ClientPredictionIntegration>() { i.semantic.local_application_failed(); }
}
pub(crate) fn observe_client_prediction<TDriver>(world: &mut World, replication: &ClientReplicationSet<ClientReplicatedStateProduct>) -> anyhow::Result<bool>
where TDriver: ReplicationDriver + InputDriver + Send + Sync + 'static, TDriver::Input: Clone + PartialEq {
    let Ok(mut integration)=world.remove_resource::<ClientPredictionIntegration>() else { return Ok(false); };
    let mut replayed_commands=0u64;
    let observation=integration.semantic.observe_replication(replication, |target,payload| {
        let commands=TDriver::decode_input(payload).map_err(anyhow::Error::new).context("decode RunenNet retained prediction batch")?;
        TDriver::apply_input(world, SimulationTick(target.get()), &commands).map_err(anyhow::Error::new).context("replay RunenNet retained prediction batch")?;
        replayed_commands=replayed_commands.saturating_add(commands.len() as u64);
        Ok::<_,anyhow::Error>(())
    });
    world.insert_resource(integration);
    match observation {
        Ok(PredictionReconciliationOutcome::ReconciledReplay{..}) => { if replayed_commands>0 && let Ok(d)=world.resource_mut::<PredictionDiagnostics>() { d.replayed=d.replayed.saturating_add(replayed_commands); } Ok(false) }
        Ok(_) => Ok(false),
        Err(PredictionReconciliationError::ReplayFailed{source,..}) => { tracing::warn!(error=%format!("{source:#}"), "client prediction replay failed; authoritative host restoration required"); Ok(true) }
        Err(error) => Err(anyhow::anyhow!("RunenNet client prediction reconciliation failed: {error:?}")),
    }
}
pub(crate) fn confirm_client_prediction_host_restored(world: &mut World, replication: &ClientReplicationSet<ClientReplicatedStateProduct>) -> anyhow::Result<()> {
    let Ok(mut integration)=world.remove_resource::<ClientPredictionIntegration>() else { return Ok(()); };
    let result=integration.semantic.confirm_host_restored_after_prediction_failure(replication).map_err(|e| anyhow::anyhow!("RunenNet prediction host-restoration confirmation failed: {e:?}"));
    world.insert_resource(integration);
    result.map(|_| ())
}
...
{
    world.resource::<NetworkInputStaging<TDriver::Input>>().context("NetworkInputStaging should be installed by NetPlugin")?;
    let authority=world.resource::<SimulationProfileConfig>().map(|c| c.authority).unwrap_or(AuthorityRole::Local);
    if matches!(authority, AuthorityRole::Client | AuthorityRole::Peer) { world.resource::<NetworkClientOutbox>().context("NetworkClientOutbox should be installed by the client network role")?; }
    if let Ok(d)=world.resource_mut::<PredictionDiagnostics>() { d.fixed_steps_observed=d.fixed_steps_observed.saturating_add(1); }
    ...
    let commands=TDriver::take_local_input(&mut world).map_err(|e| map_driver_error::<TDriver>(e,"take local input"))?;

    if matches!(authority, AuthorityRole::Client) {
        if commands.is_empty() { return Ok(()); }
        let payload=TDriver::encode_input(&commands).map_err(|e| map_driver_error::<TDriver>(e,"encode input"))?;
        ...
        if !conflicting {
            match enqueue_client_outbox(&mut world,ClientMessage::InputFrame(InputFrame{tick,payload:payload.clone()})) {
                Ok(())=>{},
                Err(NetworkPendingEnqueueError::Unavailable{endpoint,..})=>anyhow::bail!("{endpoint} should be installed by NetPlugin"),
                Err(NetworkPendingEnqueueError::Backpressure{capacity,..})=>tracing::warn!(capacity,"failed to enqueue local input frame; admitted prediction remains pending"),
            }
        }
        if matches!(outcome,Some(PredictionInputOutcome::InputAccepted)) {
            if let Err(error)=TDriver::apply_input(&mut world,tick,&commands) {
                ...
                let original=map_driver_error::<TDriver>(error,"apply newly admitted local prediction batch");
                restore_client_prediction_host_after_failure::<TDriver>(&mut world).context("restore authoritative host state after local prediction failure")?;
                return Err(original);
            }
            if let Ok(d)=world.resource_mut::<PredictionDiagnostics>() { d.commands_applied=d.commands_applied.saturating_add(commands.len() as u64); }
        }
        return Ok(());
    }
    ...
    if !commands.is_empty() {
        ...
        for command in commands {
            if let Err(NetworkInputStageError::Backpressure{capacity,..})=staging.stage(tick,command) { tracing::warn!(capacity,tick=tick.0,"network input staging backpressure; rejecting local authority input"); }
        }
    }
    let local=world.resource_mut::<NetworkInputStaging<TDriver::Input>>()?.drain_tick(tick);
    authority_inputs.extend(local);
    if authority_inputs.is_empty() { return Ok(()); }
    if let Ok(d)=world.resource_mut::<PredictionDiagnostics>() { d.commands_applied=d.commands_applied.saturating_add(authority_inputs.len() as u64); }
    TDriver::apply_input(&mut world,tick,&authority_inputs).map_err(|e| map_driver_error::<TDriver>(e,"apply streamed input"))?;
    Ok(())
}
