...
type FrameResourceCollector = for<'a> fn(&'a World, &mut RenderFrameDataRegistry<'a>);
...
fn collect_resource<'a, T>(world: &'a World, frame_data: &mut RenderFrameDataRegistry<'a>)
where
    T: Resource + Send + Sync + 'static,
{
...
mod tests {
    ...
    #[test]
    fn collect_frame_data_includes_registered_resources() {
        let mut world = World::new();
        world.insert_resource(TestResourceA(42));
        world.insert_resource(TestResourceB("ok"));
        ...
        assert_eq!(frame_data.get::<TestResourceA>(), Some(&TestResourceA(42)));
        assert_eq!(
            frame_data.get::<TestResourceB>(),
            Some(&TestResourceB("ok"))
        );
    }
