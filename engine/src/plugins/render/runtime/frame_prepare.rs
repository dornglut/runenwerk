...
pub(crate) fn build_frame_feature_contributions(
	...
	if let Ok(resource) = world.resource::<PreparedWorldFeatureResource>() {
		let world_policy =
			feature_policy(world, WORLD_DRAW_RENDER_FEATURE_ID, resource.fallback_policy);
	...
	if let Ok(resource) = world.resource::<PreparedCaveFeatureResource>() {
		let cave_policy =
			feature_policy(world, CAVE_INTERIOR_RENDER_FEATURE_ID, resource.fallback_policy);
	...
	if let Ok(resource) = world.resource::<PreparedDeformationFeatureResource>() {
		let deformation_policy =
			feature_policy(world, DEFORMATION_RENDER_FEATURE_ID, resource.fallback_policy);
	...
	if let Ok(resource) = world.resource::<PreparedWindFieldFeatureResource>() {
		let wind_policy =
			feature_policy(world, WIND_FIELDS_RENDER_FEATURE_ID, resource.fallback_policy);
