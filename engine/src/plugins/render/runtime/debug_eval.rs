...
mod tests {
	...
	fn frame_prepare_ingests_draw_material_deformation_feature_resources() {
		...
		assert!(matches!(material.payload, PreparedFeaturePayload::Material(_)));
	...
	fn prepare_inserts_missing_gate_for_execution_referenced_feature_without_payload() {
		...
		let contributions = crate::plugins::render::runtime::frame_prepare::build_frame_feature_contributions(
			&world,
			"world".to_string(),
			"overlay".to_string(),
