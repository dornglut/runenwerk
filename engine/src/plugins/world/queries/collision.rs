...
use super::super::sdf::storage::{
    SDF_PAGE_EDGE_BRICKS, SdfBrickRecord, SdfChunkPayload, SdfPageCoord3,
    WorldSdfChunkStoreResource,
};
...
pub enum WorldCollisionReadiness {
    MissingPayload { chunk_id: ChunkId },
    Ready,
}

#[derive(Debug, Copy, Clone, PartialEq, Serialize, Deserialize, ecs::Resource)]
pub enum WorldCollisionSweepOutcome {
    MissingPayload { chunk_id: ChunkId },
    Hit(WorldCollisionHit),
    Clear,
}
...
impl WorldCollisionQueryServiceResource {
    ...
    pub fn sweep_sphere(
        ...
        match self.sweep_sphere_authoritative(partition, store, planet_id, query) {
            WorldCollisionSweepOutcome::Hit(hit) => Some(hit),
            WorldCollisionSweepOutcome::MissingPayload { .. }
            | WorldCollisionSweepOutcome::Clear => None,
        }
    }
    ...
    pub fn sweep_sphere_authoritative(
        ...
        planet_id: PlanetId,
        query: SphereSweepQuery,
    ) -> WorldCollisionSweepOutcome {
        let required_chunks = required_chunks_for_sweep(partition, planet_id, query);
        if let Some(chunk_id) = first_missing_payload_chunk(store, &required_chunks) {
            return WorldCollisionSweepOutcome::MissingPayload { chunk_id };
        }
        ...
        WorldCollisionSweepOutcome::Clear
    }
}
...
fn collides_at_sweep_t(
    service: &WorldCollisionQueryServiceResource,
    partition: &WorldPartitionConfig,
    store: &WorldSdfChunkStoreResource,
    planet_id: PlanetId,
    query: &SphereSweepQuery,
...
mod tests {
    use super::*;
    use crate::plugins::world::ids::{ChunkGeneration, ChunkRevision};
    use crate::plugins::world::sdf::storage::{
    ...
    fn sweep_readiness_reports_first_missing_chunk_in_path_bounds() {
        let service = WorldCollisionQueryServiceResource;
        ...
        assert_eq!(
            readiness,
            WorldCollisionReadiness::MissingPayload {
    ...
    fn sweep_readiness_is_ready_when_all_required_chunks_are_loaded() {
        ...
        assert_eq!(readiness, WorldCollisionReadiness::Ready);
    }
    ...
    #[test]
    fn sweep_returns_none_when_readiness_is_missing_for_swept_chunk() {
        let service = WorldCollisionQueryServiceResource;
        let partition = WorldPartitionConfig {
            chunk_edge_meters: 1.0,
            region_chunk_dims: [8, 8, 8],
        ...
        assert!(
            hit.is_none(),
            "sweep should fail-safe when required chunk payloads are missing"
        );
    }
    ...
    fn authoritative_sweep_outcome_distinguishes_clear_and_missing_payload() {
        ...
        let clear_query = SphereSweepQuery {
            start: [0.1, 0.1, 0.1],
            end: [1.8, 0.1, 0.1],
            radius: 0.0,
        };
        let mut clear_store = WorldSdfChunkStoreResource::default();
        clear_store
            .chunks
        ...
        assert_eq!(clear_outcome, WorldCollisionSweepOutcome::Clear);

        let missing_query = SphereSweepQuery {
            start: [0.1, 0.1, 0.1],
            end: [2.8, 0.1, 0.1],
            radius: 0.0,
        };
        let mut missing_store = WorldSdfChunkStoreResource::default();
        missing_store
            .chunks
            .insert(clear_chunk0, clear_payload(clear_chunk0));
        let chunk2 = ChunkId::new(planet_id, ChunkCoord3 { x: 2, y: 0, z: 0 });
