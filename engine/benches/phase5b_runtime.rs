...
impl Plugin for Phase5bRuntimeBenchPlugin {
...
fn workload_engine_runtime(c: &mut Criterion) {
    let mut group = c.benchmark_group("engine_runtime_phase5b");

    for &entity_count in &[5_000_usize, 20_000] {
        let mut app = App::headless();
        app.add_plugin(Phase5bRuntimeBenchPlugin { entity_count });
...
criterion_group!(phase5b_runtime, workload_engine_runtime);
criterion_main!(phase5b_runtime);
