...
fn build_flow() -> RenderFlow {
    RenderFlow::new("v2.flow")
        ...
        .uniform_from_state::<FlowState, ComputeParams>(FlowState::compute_params)
        ...
        .uniform_from_state_with_surface::<FlowState, ComposeParams>(FlowState::compose_params)
        .bind_ping_pong_storage("cells")
        .write_surface_color()
        .depends_on("simulate")
        .finish()
...
fn shader_registry_supports_asset_and_explicit_registration() {
    ...
    let b = registry.register_shader_with_id(
        "custom.compose",
