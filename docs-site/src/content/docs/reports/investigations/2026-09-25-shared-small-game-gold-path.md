...
- #942 — documentation publication-class authoring policy;
- #940 — normalized input-trace replay;
- #915 — Editor shell workspace-state retirement.

The report is documentation-only and does not take ownership of those implementation paths. Accepted
#944 has already moved the local simulation/replay packages to `domain/simulation` and
`domain/replay` while retaining the crate-facing `engine_sim` / `engine_replay` contracts used by
Engine integration. Accepted #931 has also landed the temporal-resolution work. Any implementation
derived from this report must still re-resolve then-current accepted state and active writers before
publication.
