...
- `RenderFixedResolutionExecutionRequest` is a return-only renderer helper. Its public preparation path validates a sub-native same-aspect internal extent, requires the selected compiled flow to use automatic-main routing, requires the selected color-target alias to have writers valid on both native-main and offscreen views, and validates the compiled renderer-owned resolve flow rather than accepting an unchecked resolve-flow ID. A valid Fixed admission then prepares a sampleable RGBA dynamic target, offscreen view, selected-flow invocation, native-output resolve invocation, and automatic-main replacement claim.
...
- `inspect_fixed_resolution_execution(...)` is the execution-evidence gate. It reports `policy = Fixed` only when the actual `PreparedRenderFrame` contains the admitted native output extent, internal view, exact dynamic-target descriptor, scene invocation/binding, resolve invocation/binding, and no duplicate native-main scene invocation. Native fallback is accepted only when the actual frame has the expected native-main scene invocation and retains none of the fixed internal view/target/scene/resolve identities.
...
- `fixed_resolution_resolve_flow()` provides the canonical portable fullscreen spatial resolve to native `SurfaceColor`. Fixed admission validates that compiled resolve contract before publishing the resolve invocation; a same-shaped label substitute or unrelated flow is not accepted. It does not claim TAAU reconstruction quality or choose a production scale.
...
- production readiness inspection:
  ...
  - `RenderTemporalResolutionPolicy`
...
- `inspect_render_temporal_inputs(...)` reports WR-070 temporal input availability, explicit native/fixed/dynamic internal/output resolution policy, jitter sequence and phase, history resource/signature validity, reconstruction mode, native fallback state, and fail-closed diagnostics. Missing required inputs, native-resolution extent mismatches, invalid dynamic bounds, invalid history signatures, invalid jitter evidence, TAAU without a scaled fixed/dynamic policy, and temporal reconstruction with invalid history and no native fallback are explicit diagnostics. The report is renderer execution evidence only; camera, scene, product, exposure, material reactivity, SDF, ray-query, freshness, fallback legality, and authority truth remain producer-owned.
