...
- [ ] Migrate retained UI slices to Interaction V2 contracts. Status: active/current with the first code-bearing retained slice landed for `IV2-menu-stack` and `IV2-scroll-ownership`. `domain/ui/ui_definition/src/interaction.rs`, `domain/ui/ui_definition/src/form.rs::form_retained_ui`, `domain/ui/ui_runtime/src/input/pointer.rs::dispatch_pointer_event`, `domain/ui/ui_runtime/src/runtime/ui_runtime.rs::dispatch_keyboard_event`, and `domain/editor/editor_shell/src/composition/build_editor_shell.rs::build_editor_shell_frame_with_docking_visual_state` now carry the generic menu-stack, focus-return, popup-layer, Escape-dismiss, and scroll-boundary ownership spine. Remaining slices are `IV2-menu-sizing`, `IV2-chrome-slots`, `IV2-dock-drop-zones`, and `IV2-status-and-viewport-arbitration`; viewport tools/options/details adapters still need to consume the landed contracts before local shell polish expands.
