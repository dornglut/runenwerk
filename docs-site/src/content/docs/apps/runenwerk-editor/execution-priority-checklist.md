...
- [ ] Complete the integrated UI/editor live replacement and asset foundation. Status: Active; M4A-M4E active editor-definition consumption and reusable-control cleanup are implemented as of 2026-05-09. The app now has an owned known-command resolver, transactional command-binding and shortcut activation, active shortcut dispatch, active in-editor menu/toolbar consumption, app-supplied route-action projection, panel default tool-surface compatibility validation, active tool-surface ordering for future switch/create choices, shared `editor_shell` reusable-control composition helpers, and provider guards against direct app-side reusable-control construction. The asset pipeline should start inside this track so Asset Browser, Import Inspector, Field Product Viewer, and SDF Brush Browser surfaces enter through active editor-definition catalogs instead of new hard-coded shell UI.
  - Next order: `domain/asset` and `ProjectFileV2` contracts first; first asset runtime/catalog surfaces second; import/field-product jobs third; viewport producer and history breadth fourth.
