...
- [x] Implement the M3.6 UI self-authoring workspace and styling before M4. Status: complete as of 2026-05-06 for authored definition editing, retained previews, and apply/rollback snapshots; applied theme definitions now activate the live editor host theme, while UI template/workspace/menu/shortcut/command-binding/panel/tool-surface definitions still need explicit live activation seams.
...
- [ ] Continue Editor Design/self-authoring polish for UI/editor layout design, creation, and management. Status: post-M3.6 follow-up; exhaustive field-by-field controls, packaging, and exchange hardening remain outside the M3.6 completion scope while M4 asset-pipeline work starts from the available self-authoring substrate.
