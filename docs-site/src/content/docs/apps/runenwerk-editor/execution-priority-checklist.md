...
- [x] Preserve and extend UI/editor guard coverage for structural routing, capability gating, and seam ownership. Status: updated on 2026-05-08; architecture guards cover provider seams, `ui_definition` behavior isolation, viewport presentation ratification, inspector enum routing, reusable viewport toggle routing, self-authoring live activation formation, and viewport product catalog expansion.
- [ ] Keep editor/UI cross-doc sequencing aligned with shipped behavior. Status: Active.
- [ ] Complete the integrated UI/editor live replacement and asset foundation. Status: Open/next; active catalogs exist for UI templates, editor bindings, menus, shortcuts, command bindings, panel registries, and tool-surface registries, but menu consumption, shortcut dispatch, command-binding resolution, and projected panel/tool-surface choices still need to use those active catalogs end to end. The asset pipeline should start inside this track so Asset Browser, Import Inspector, Field Product Viewer, and SDF Brush Browser surfaces enter through active editor-definition catalogs instead of new hard-coded shell UI.
  - Next order: command-binding resolver first; active shortcut dispatch second; active menu/toolbar consumption third; active panel/tool-surface registry projection fourth; reusable-control cleanup fifth; `domain/asset` and `ProjectFileV2` contracts sixth; first asset runtime/catalog surfaces seventh; import/field-product jobs eighth; viewport producer and history breadth ninth.
