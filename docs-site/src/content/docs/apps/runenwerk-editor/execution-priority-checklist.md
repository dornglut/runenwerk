...
Use the linked current-lifecycle docs as authoritative details.
...
- Completed predecessor-shaped milestones below are implementation/history evidence, not permission to reassert one universal `EditorSession`, workspace graph, selection, history, or persistence owner. ADR 0013 owns structural composition and ADR 0025 owns normalized editor coordination.
...
- [x] Phase A: Introduce workspace profile abstraction without breaking MVP. Status: Automated verified by shell state and workspace profile tests; current workspace-era types are compatibility/implementation evidence, while live structural authority is `ui_composition`.
- [x] Phase B: Decouple workspace layout persistence from scene path coupling. Status: Automated verified by workspace layout persistence tests; current composition persistence is governed by `ui_composition`.
- [x] Phase C: Formalize document tabs and active document switching. Status: Implemented and test-covered in the predecessor model; current `EditorSession` owns ordered document tabs, active document switching, dirty/save/close transitions, and document compatibility validation, with app-local generic document-tab runtime state split from scene runtime document state. ADR 0025 supersedes that shape as the normalized future ownership model.
- [x] Phase D: Replace adapter-only panel wiring with provider registry routing. Status: Implemented and test-covered; provider DTOs carry current workspace/profile and semantic context, surface definition, capabilities, and provider-local routes, and concrete scene/console providers are split into provider subdomain modules. Structural placement remains owned by `ui_composition`.
- [x] Close editor app-domain operation migration seams for ratification, scene command execution, transaction orchestration, history, scene, and selection ownership. Status: Implemented for M1; domain-owned scene operation functions execute single commands and transactions with history insertion, while app-owned ECS/reflection, snapshots, retention, projection parity, selection sync, and recording remain in the app layer. This is current implementation evidence, not a universal cross-domain history/selection owner.
- [x] Phase E: Expand global mode enum into scoped workspace/document mode sets. Status: Implemented and test-covered in current code; `editor_core` uses mode ids, descriptors, mode registry, and workspace/document compatibility validation. ADR 0025 does not require one global normalized active mode.
- [x] Complete UI docking/tab behavior on top of existing structural identity and binding contracts. Status: Implemented and test-covered; tab reorder, rehome, floating host creation, split resizing, area split/duplicate/reset/close, dynamic split-area composition, and structural identity preservation have automated coverage. Current structural mutation enters the `ui_composition` transaction path.
...
- [x] Add plus/new-tab affordance for creating a new tab in a tab stack. Status: Implemented and test-covered; tab chrome exposes a plus/new-tab control through current workspace-era identity/adaptation code. The resulting live structural mutation is governed by `ui_composition` rather than the compatibility allocator becoming structural authority.
...
- [x] Implement the M3.6 UI self-authoring workspace and styling before M4. Status: complete as of 2026-05-06 for authored definition editing, retained previews, and apply/rollback snapshots; applied theme definitions activate the live editor host theme, and applied workspace layout definitions form candidate composition state that is installed through the current composition boundary rather than becoming an independent writable workspace authority. Exported definitions use a versioned package. The 2026-05-08 maturity pass also activates UI template, editor binding, menu, shortcut, command-binding, panel-registry, and tool-surface-registry catalogs without moving provider behavior into `ui_definition`.
