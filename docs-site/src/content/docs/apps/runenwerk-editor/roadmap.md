...
- Native multi-window foundations are substantially implemented: logical/native/render target binding, detach-to-new-target, atomic persisted multi-target restore, and profile-owned fresh `Window > New Window` target formation all use the composition transition coordinator. MW6 all-target viewport/product projection and secondary-window camera/picking/input product proof remain before second-monitor workflows are product-ready.
