---
title: UI Component Platform NodeCanvas Design
description: Generic node/link view contract for skill trees, tech trees, dialogue trees, behavior trees, quest trees, and graph viewers.
status: active
owner: ui
layer: domain
canonical: true
...
# UI Component Platform NodeCanvas Design
...
`NodeCanvas` owns generic node/link presentation. Ports, graph editing, and domain semantics live in higher layers or products.

## Feature List

- node identity, state, and bounds
- directed and undirected links
- link endpoints, labels, and routing
- node groups and collapsed groups
- locked, available, invalid node states
- focus path and dependency path highlight
- large graph/tree budgets

## Ergonomics Gate

An endless skill tree and a graph viewer can share node/link visualization without either becoming a port graph editor.
