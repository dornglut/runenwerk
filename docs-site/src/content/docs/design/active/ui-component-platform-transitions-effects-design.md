---
title: UI Component Platform Transitions And Effects Design
description: Generic component transition/effect primitives with state timeline, reduced motion, diagnostics, and renderer-boundary evidence.
status: active
owner: ui
layer: domain
canonical: true
...
# UI Component Platform Transitions And Effects Design
...
Transitions and effects are generic state behavior with story evidence and reduced-motion policy, not renderer flourish.

## Feature List

- canonical state timeline
- hover, pressed, focus-ring, selected, disabled, error transitions
- popup open/close transitions
- reduced motion
- timing evidence
- effect diagnostics
- renderer primitive boundary
- unsupported effect expected failures

## Ergonomics Gate

A designer can inspect supported transitions, token requirements, timing facts, and reduced-motion behavior.

## Out Of Scope

- world-space visibility
- entity-attached projection
- game damage/nameplate effects
- Designer product animation tools
- renderer-authored UI semantics
