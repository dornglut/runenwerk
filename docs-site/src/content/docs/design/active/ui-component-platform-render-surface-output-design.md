...
description: Renderer-neutral primitive and surface output contract for reusable controls and surface packages.
status: active
owner: ui
layer: domain
canonical: true
...
Control output lowers to renderer-neutral primitives or surface packets with provenance. Renderer code does not own UI source truth.

## Feature List

- rect, border, clip, image, glyph run, stroke
- graph canvas primitive batch
- product surface primitive
- viewport surface embed
- layer, sort key, draw key, primitive order, surface order
...
Render debugging can show which control/story produced which primitives and why.

## Out Of Scope

- renderer-authored UI semantics
- backend-specific control behavior
