...
description: Templates, naming conventions, checklists, and examples for adding ControlPackages without guessing platform structure.
status: active
owner: ui
layer: domain
canonical: true
...
Control authoring must be ergonomic. PT-UI-COMPONENT-PLATFORM needs reusable templates and conventions so new controls are easy to define, prove, inspect, and migrate.

## Feature List

- package template
- control kind naming convention
- property/state/event schema naming conventions
- route, diagnostic, fixture, and story id conventions
- default component story matrix template
- expected-failure template
- accessibility/token/binding/mount eligibility checklists
- examples for simple controls, data controls, and surface controls
- migration and deprecation template

## Ergonomics Gate

A human or implementation agent can add a new ControlPackage without reading unrelated runtime, story, designer, or gallery code.

## Out Of Scope

- generating product UI automatically
- replacing UI Designer product authoring
- bypassing story proof
