...
ADR 0020 and the canonical Runenwerk Platform Architecture own durable accepted
platform decisions. This active design elaborates the detailed target. Unresolved
public API spelling, repository extraction, implementation sequencing, and provider
details are not implementation authorization. Where this design conflicts with an
accepted ADR or the canonical platform spine, the accepted ADR and platform spine win.
...
## Status and implementation honesty

This is an accepted architectural North Star, not a claim that the Plan IR, semantic
catalog, planner, providers, dialects, or Workbench integration exist in current code.
Implementation remains separately gated and should proceed only through bounded,
reviewed slices.
...
It is not a universal physical store, global identity system, global revision or
transaction authority, mandatory materialization format, or owner of private framework
state. Inspection correlation through the catalog does not certify runtime admission.
...
Workbench explanation and correlation do not grant foreign mutation authority or
runtime admission authority. The consuming owner or Runenwerk integration boundary
still decides whether owner-local versions, capabilities, provenance, quality, and
lifetime facts form a legal runtime input set.

# 16. Product composition and ergonomics

Application composition may itself use Plan fragments, so named product recipes remain inspectable compositions rather than opaque runtime presets. Product/application Plans lower through Runenwerk product composition into the existing `App`, plugins, resources, adapters, owner calls, and execution work. `App` remains the one live runtime composition root.

This target is not implemented today. A product Plan must not remain as a persistent
parallel application-configuration authority, second runtime, or service locator after
lowering.
...
This adoption does not create `runen-plan`, `runen-semantic`, or another crate or
repository, and it does not change dependency topology. Initial Plan participation may
remain Runenwerk-owned integration and providers. Future extraction requires a separate
ADR under ADR 0014, concrete implementation evidence, and at least one independent
consumer proving the reusable boundary.
...
This North Star generalizes them: future semantic-graph and gameplay authoring may
compile into the typed Plan IR rather than defining a competing universal IR, while
ECS/scheduler products become possible physical lowerings rather than the defining
gameplay execution ontology.
...
Later implementation should advance only through this evidence sequence:

1. build a semantic API and representative workload corpus;
2. design the minimal typed Plan IR and dialect mechanism needed by that corpus;
3. prove planner/provider interfaces with deterministic straightforward lowering;
4. prove a world-plus-ECS path without transferring world or ECS authority;
5. prove a structurally different second provider with owner-native realization;
6. only then consider extraction of reusable Plan infrastructure under ADR 0014.

Optimizer sophistication is not a correctness dependency. Deterministic
straightforward lowering is a valid complete baseline.
