...
description: Generic 2D coordinate, navigation, transform, bounds, overlay, and large-content primitive for reusable surfaces.
status: active
owner: ui
layer: domain
canonical: true
...
`Surface2D` is the generic 2D coordinate/navigation primitive. It is not Gallery-specific and not GraphCanvas-specific.

## Feature List

- surface id
- content bounds and viewport bounds
- world-to-screen and screen-to-world transforms
- pan, zoom, zoom-to-cursor
- fit all, fit selection, fit content
- selection rectangle and hover coordinate
- pointer capture and gesture cancel/commit
- overlay layer and diagnostic overlay
- grid/background
- large-content bounds, LOD readiness, budget evidence
- invalid transform expected failure

## Consumers

- Gallery catalog inspection
- UI Designer canvas
- SpatialCanvas, NodeCanvas, PortGraphCanvas
- TrackSurface, Timeline, CurveEditor
- Material, SDF, gameplay, animation, and particle graphs
