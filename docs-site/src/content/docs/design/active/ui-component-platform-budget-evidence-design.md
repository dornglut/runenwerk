---
title: UI Component Platform Budget Evidence Design
description: Reusable performance and scale evidence vocabulary for controls, containers, Surface2D, GraphCanvas, and Timeline.
status: active
owner: ui
layer: domain
canonical: true
...
# UI Component Platform Budget Evidence Design
...
Large data and large surface readiness need shared budget evidence before specialized components claim runtime maturity.
...
- glyph run count
- interaction trace step count
- list/table/tree item count
- virtualized item count
- canvas object count
- graph node/edge/port count
- timeline track/keyframe count
- frame build budget class
- large scene expected diagnostics
- unsupported budget expected failure

## Ergonomics Gate

Gallery/CLI report whether a component passed at toy size, expected size, or large-scene budget level.

## Out Of Scope

- renderer GPU scheduling policy
- game simulation budgets
