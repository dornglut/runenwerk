...
description: Supersession marker for the pre-ADR-0017 Domain Workbench Meta Kernel and universal Domain Program target.
status: superseded
owner: workspace
layer: history
canonical: false
...
The old implementation ontology is superseded. In particular, do **not** use this path
to justify:
...
Several current accepted/supporting documents still contain historical `related_docs`
links to this path. Repository code search is unavailable during the #251 cutover, so
this path remains only as an explicit non-authoritative supersession marker instead of
risking unverified broken links.

Removal condition: a later #205 reference/lifecycle cleanup may delete this marker after
all current inbound references have been command-verified and migrated. It must never
return to active or canonical architecture authority.
