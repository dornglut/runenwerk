...
| WR-025 | UI Runtime V2 and interaction formation | P0 | V5 | B2 | 4.8 | Accepted architecture gate with runtime-backed slices implemented for IV2-menu-stack, IV2-scroll-ownership, and IV2-menu-sizing. WR-024 shell polish may consume only those landed contracts, while IV2-chrome-slots, dock/drop-zone, and status/viewport arbitration remain WR-025-gated. | Use the landed generic menu-stack, scroll-ownership, and menu-sizing spine before WR-024 touches popup dismissal, focus return, popup layering, submenu anchoring, viewport wheel fallback, popup clamp, or item fill; define the next named Interaction V2 contract slice before extending shell polish beyond those behaviors. |
