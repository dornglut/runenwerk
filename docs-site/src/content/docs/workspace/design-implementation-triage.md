...
| WR-173 | Artifact-Backed Static UI Gallery Runtime Rendering | P0 | V4 | B3 | 2.6 | Use this row only for the static basic/selected button gallery. Interactivity, hover/pressed/focus loops, inspector UI, additional controls, and production-readiness evidence require follow-on rows. | Closeout evidence is still required before claiming the production milestone complete. |
