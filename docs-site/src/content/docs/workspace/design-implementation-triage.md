...
| WR-020 | Source-backed Asset Core Contracts | P0 | V5 | B3 | 1.9 | Supporting foundation for Field Visualizer, Material Lab, and prefab V2. Editor app wiring is downstream WR-026 adapter work, not semantic asset truth. | Source identity must wait for rendered-world product usage evidence and avoid reopening broad external import scope; domain/asset contracts must land before editor adapters expand. |
| WR-026 | Source-backed Asset Editor Adapters | P1 | V4 | B3 | 2.4 | Implement only after WR-020 lands the domain-owned source/catalog contracts. | Editor adapters must wait for domain/asset descriptor, import-plan, dependency, diagnostics, and ratification contracts to stabilize. |
