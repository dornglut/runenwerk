...
| WR-025 | UI Runtime V2 and interaction formation | P0 | V5 | B2 | 4.8 | Accepted architecture gate with runtime-backed slices implemented for IV2-menu-stack, IV2-scroll-ownership, IV2-menu-sizing, and IV2-chrome-slots. WR-024 shell polish may consume only those landed contracts, while dock/drop-zone and status/viewport arbitration remain WR-025-gated. | Use the landed generic menu-stack, scroll-ownership, menu-sizing, and chrome-slot spine before WR-024 touches popup dismissal, focus return, popup layering, submenu anchoring, viewport wheel fallback, popup clamp, item fill, or tab/workspace close and active-indicator placement; define the next named Interaction V2 contract slice before extending shell polish beyond those behaviors. |
