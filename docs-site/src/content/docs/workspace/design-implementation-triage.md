...
| WR-025 | UI Runtime V2 and interaction formation | P0 | V5 | B2 | 4.8 | Accepted architecture gate with runtime-backed slices implemented for IV2-menu-stack, IV2-scroll-ownership, IV2-menu-sizing, IV2-chrome-slots, and IV2-dock-drop-zones. WR-024 shell polish may consume only those landed contracts, while status/viewport arbitration remains WR-025-gated. | Use the landed generic menu-stack, scroll-ownership, menu-sizing, chrome-slot, and dock/drop-zone spine before WR-024 touches popup dismissal, focus return, popup layering, submenu anchoring, viewport wheel fallback, popup clamp, item fill, tab/workspace close and active-indicator placement, or tab reorder/drop-zone precedence; define status/viewport arbitration before status or viewport-input polish. |
...
| WR-024 | Editor Shell Polish | P0 | V4 | B1 | 4.0 | Ready-next only after the Interaction V2 contract/migration spine leads; route long-term popup, scroll, chrome, docking, and status overflow decisions through WR-025, consuming only landed slices. | Interaction V2 contract/migration spine must lead; polish can only proceed as a retained-UI slice consuming those contracts or as explicitly bounded compatibility evidence. |
