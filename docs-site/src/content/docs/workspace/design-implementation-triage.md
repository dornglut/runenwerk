...
|---|---|---:|---:|---:|---:|---|---|
| WR-019 | Field Visualizer product workflow | P1 | V4 | B2 | 2.4 | Plan immediately after rendered-world V1 stabilizes the packet/product handoff; this is the first FR-2 product-producer follow-up in the fully featured renderer roadmap. | Implement from field-visualizer-product-workflow-design.md using existing viewport product target state and the FR-2 sequencing in engine/roadmaps/fully-featured-renderer-roadmap.md. |
...
| WR-026 | Source-backed Asset Editor Adapters | P1 | V4 | B3 | 2.4 | Ready-next only after the WR-020 closeout; do not implement in the WR-020 thread. Adapter outputs feed later renderer consumers but do not move asset truth into renderer code. | Explicit user direction excludes WR-026 from this objective. Editor adapter load/save, import orchestration, diagnostics surfacing, and prior-valid artifact UI need a separate selected roadmap slice. |
| WR-021 | Material Lab and material preview products | P1 | V4 | B2 | 0.9 | Do not make canvas state material truth; material source documents own material state and renderer code consumes prepared preview/material products only. | Material preview work waits for rendered-world V1, Field Visualizer routing, and source-backed asset adapter evidence. |
