...
- [ ] Execute remaining SDF-first Phase 6 / M6.2 procgen proof work. Status: active after Phase 6A; `domain/procgen` now owns the domain-first procgen product track for graph-backed deterministic documents, terrain/material node semantics, ratification, lowering to `world_ops` operation windows, planning metadata, reservations, explanations, and product job/publication descriptors. Remaining work must add editor/runtime proof only through the completed product substrate (source: `workspace/sdf-first-execution-roadmap.md`).
...
- [ ] Sequence procedural authoring domains for material/texturing, procgen, particles, physics, animation, and simulation. Status: gated by the SDF-first open-world substrate roadmap; remaining M6.2 procgen editor/runtime proof must consume `domain/procgen` plus the completed publication barrier, query snapshot, strict consumer, render selection, and derived GPU residency substrate (source: `workspace/sdf-first-execution-roadmap.md` and `design/active/editor-procedural-content-and-simulation-workflow-plan.md`).
...
- [ ] Plan Phase 6B / M6.2 editor-runtime procgen proof from the Phase 6A `domain/procgen` contracts. Scope remains bounded region terrain/material generation, product publication, query snapshots, render selection, derived residency, preview/bake commands, and editor diagnostics without concrete generator algorithms (source: `workspace/sdf-first-execution-roadmap.md`, `domain/procgen/README.md`, and `apps/runenwerk-editor/roadmap.md`).
...
- [x] Complete SDF-first execution Phase 5: procgen readiness gate. Status: accepted and validated as of 2026-05-13; `domain/procgen/README.md` defines graph-backed generator documents, procgen-owned planning lifecycle, reservation boundaries, seed/scope/version policy, cache lineage, authored overlay preservation, runtime/offline policy, server-validated multiplayer authority, and product output paths. Phase 6A later implemented the domain crate while providers, preview, bake, and generator algorithms remain deferred (source: `workspace/sdf-first-execution-roadmap.md` and `reports/closeouts/sdf-first-execution-phase-5/closeout.md`).
