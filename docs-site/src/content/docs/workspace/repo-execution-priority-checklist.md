...
- [ ] Execute SDF-first Phase 6 / M6.2 as the first procgen product-domain implementation. Status: active; Batch 1 product contract alignment plus Phases 1 through 5 are complete, and M6.2 procgen code may start only from the accepted `domain/procgen/README.md` contract, including its procgen-owned planning lifecycle and reservation boundaries (source: `workspace/sdf-first-execution-roadmap.md`).
...
- [ ] Keep M6.2 procgen scoped to the first accepted product track: bounded region terrain/material generation, planning lifecycle metadata, reservations, explanation data, lowering to `world_ops` operation windows, and changed-region diagnostics (source: `workspace/sdf-first-execution-roadmap.md`, `domain/procgen/README.md`, and `apps/runenwerk-editor/roadmap.md`).
...
- [x] Complete SDF-first execution Phase 5: procgen readiness gate. Status: accepted and validated as of 2026-05-13; `domain/procgen/README.md` defines graph-backed generator documents, procgen-owned planning lifecycle, reservation boundaries, seed/scope/version policy, cache lineage, authored overlay preservation, runtime/offline policy, server-validated multiplayer authority, and product output paths while `domain/procgen` code, providers, preview, bake, and generator algorithms remain deferred to Phase 6 (source: `workspace/sdf-first-execution-roadmap.md` and `reports/closeouts/sdf-first-execution-phase-5/closeout.md`).
