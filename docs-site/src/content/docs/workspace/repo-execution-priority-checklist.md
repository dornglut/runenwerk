...
- [ ] Plan and execute SDF-first Phase 6D / M6.2C procgen bake, rollback, persistence, and runtime preview reload classification. Status: active after Phase 6C; deterministic terrain/material CPU field previews now publish formed preview descriptors through product publication barriers and query snapshots. Next work must keep renderer rebuilds, GPU upload, caves, stamps, scatter, worker pools, and broad runtime execution deferred unless the owning roadmap advances (source: `workspace/sdf-first-execution-roadmap.md`).
...
- [ ] Sequence procedural authoring domains for material/texturing, procgen, particles, physics, animation, and simulation. Status: gated by the SDF-first open-world substrate roadmap; remaining M6.2 procgen bake, persistence, rollback, and reload work must consume `domain/procgen` plus the completed publication barrier, query snapshot, strict consumer, render selection, derived GPU residency, Phase 6B editor/runtime overlay proof, and Phase 6C formed CPU preview substrate (source: `workspace/sdf-first-execution-roadmap.md` and `design/active/editor-procedural-content-and-simulation-workflow-plan.md`).
