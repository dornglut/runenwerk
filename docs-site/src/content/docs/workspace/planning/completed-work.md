...
- Planning completion alone did not prove Rust implementation; issue `#177` owns the
  separately bounded implementation record below.

### G3 implementation candidate

- Issue `#177` / draft PR `#181` implemented checked buffer, texture/view, and query
  access; typed operations; region-aware initialization; exact hazards; typed
  cross-fragment causality; immutable fragments/nodes; deterministic preparation;
  and operation-derived requirements.
- The branch migrates render planning/runtime, inspection, transient planning,
