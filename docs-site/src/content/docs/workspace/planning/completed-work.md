...
- `PT-UI-RUNTIME-PLATFORM-010` UiPlugin Render Publication: completed 2026-07-07 through merged PR #101 at merge commit `8d6c13146deab870dca5533204067249aa2c1b90`. Closeout report: `../../reports/closeouts/pt-ui-runtime-platform-010-closeout.md`. Next safe action is `PT-UI-RUNTIME-PLATFORM-011 — Scene/Debug Overlay Producer Migration and Retirement` planning only; implementation remains blocked until separately authorized.
...
Follow-up: Fulfilled by completed `PT-UI-RUNTIME-PLATFORM-005 — Typed Screen /
Source / Action Contracts` through PR #85 and closeout truth. Use Phase 006
planning as the next runtime-platform state.

## PT-UI-RUNTIME-PLATFORM-005 evidence

Implementation evidence: PR #85 delivered the engine-owned Typed Screen / Source
/ Action Contracts facade. Evidence covers `UiTypedScreenId`, `UiScreen`,
`IntoUi`, `UiTypedSource`, source lowering through `ui_program_lowering`,
route/source-map fact exposure, `UiTypedActionId`, `UiTypedActionDescriptor`,
`UiAction`, `UiHostMutationIntent`, `UiActionHandler`, typed-contract
diagnostics, focused `ui_typed_*` engine tests, and comparison evidence from
`ui_app_integration`.

Merge evidence: PR #85, `PT-UI-RUNTIME-PLATFORM-005 Typed Contracts`,
squash-merged into `main` on 2026-07-07 at merge commit
`6226470defa7a72a567fc03c1bc3783e63e2c2c8`.

Validation: Before merge, `cargo test -p engine ui_typed`, `cargo test -p
engine`, `python tools/docs/validate_docs.py`, `git diff --check`, `git diff
--check main...HEAD`, `git status --short --branch`, and `git diff --stat
main...HEAD` passed. GitHub reported no checks for the branch.

Known non-goals: mounted session runtime, host action dispatch, runtime trace,
render adapters, SurfaceFrame generic producer-boundary implementation,
scene/debug overlay migration, source reload/persistence,
`apps/ui_counter_runtime`, SDF/world-space/SpatialCanvas, `foundation/meta`,
`domain/app_program`, generic plugin framework, phase spec validator, and
tools/docs validator changes remained out of scope.
