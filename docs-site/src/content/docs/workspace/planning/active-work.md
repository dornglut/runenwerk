...
related_docs:
  ...
  - ../../design/active/ui-component-platform-overlay-popup-layering-implementation-amendment.md
---
...
Known blockers: Stop and revise scope if implementation requires files outside the accepted/amended expected file list, editor shell surface registration, Workbench provider redesign, command execution in generic UI, product/editor/game mutation in generic UI, app-specific modal lifecycle in generic UI, full text editing, dynamic plugin framework, `foundation/meta`, generic plugin primitives, phase-shaped public API names, compatibility-only aliases/shims, or overlay behavior below `ui_runtime::input`.

Next action: Validate the amended implementation locally, fix compile/test/docs issues, and only then record closeout evidence. Do not mark the phase completed until implementation and validation evidence are recorded.

Evidence: PR #43 is closed and merged into `main` at merge commit `c8b73dfa95fc335fd2b33c9137cac03a0f35060f`. User start condition reports PR #43 validated and merged. PR #44 records the Phase 13 design intake, the follow-up ergonomics/flexibility/extensibility pass, and the implementation placement amendment. User correction on 2026-07-01 requires PR #44 to close the full Phase 13, not merge as design-only.
