...
Write scope: planning and design intake only. Define owner boundaries, acceptance criteria, validation gate, stop conditions, and the exact later implementation PR envelope for generic reusable interaction. Do not change product code, do not implement interaction runtime behavior, do not add overlays/popups/layering, do not add text editing, do not add app/editor/game-specific mutation, do not extract a shared plugin framework, and do not introduce `foundation/meta`.
...
Next action: Review the Phase 12 Generic Interaction design intake and decide whether to authorize a narrow implementation PR. Keep overlays/popups/layering in Phase 13 and text editing in later phases.
