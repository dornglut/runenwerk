...
| WR-048 | Render Prepared-Frame Preflight Cache And Timing Repair | Core support | current_candidate | not_applicable | L1 | Governed render regression repair | 4 | 2 | 3 | 4 | 4 | 8 | 0.8 | 1.5 | Repair render-flow runtime regression without weakening typed compiler/preflight contracts. | Performance | Implement cached prepared-frame structural preflight, strict validation control, public preflight/flow encode timings, inspection evidence, benchmark coverage, and closeout validation. | Current bounded render regression repair row. Full preflight remains the correctness authority, but unchanged prepared-frame structures must use cached successful preflight reports in steady state. |
