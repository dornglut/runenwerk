...
| WR-025 | UI Runtime V2 and interaction formation | Architecture guardrail | current_candidate | L1 | Accepted architecture gate | 5 | 2 | 4 | 5 | 4 | 3 | 0.8 | 4.8 | Prevent repeated editor UI contract regressions | Basic | Remaining retained UI migration slices after code-bearing IV2-menu-stack, IV2-scroll-ownership, IV2-menu-sizing, IV2-chrome-slots, and IV2-dock-drop-zones guard slices landed; next work should consume those contracts for WR-024 shell polish or define the remaining status/viewport arbitration slice before implementation. | Accepted ADR 0009 inserts an execution-neutral interaction formation layer before retained UI products while keeping retained UI as the first execution target and renderer output as derived product data. |
