...
| WR-009 | Native multi-window editor presentation | Productization | ready_next | not_applicable | L2 | Runtime gate | 3 | 3 | 2 | 3 | 3 | 8 | 0.5 | 0.7 | Candidate after workflow reach exists | Performance | Window-scoped runtime, input, UI frame, and swapchain ownership proof. | Keep gated behind runtime/window-scope contracts. |
| WR-010 | Render fragment and data-driven maturity R10 | Productization | ready_next | not_applicable | L2 | Product priority gate | 3 | 3 | 2 | 3 | 3 | 8 | 0.5 | 0.7 | N/A | Neutral | Stable aliases, prepared flow invocation, hot reload, diagnostics, and inspection evidence. | Queue as FR-7 after render surface, ergonomics, persistence, and inspection priorities. |
...
| WR-030 | Model Mesh Renderable Scene Contract | Product workflow | current_candidate | not_applicable | L3 | Current Mesh Preview pixel proof | 4 | 2 | 4 | 4 | 4 | 8 | 0.5 | 1.0 | Provide the source-backed model/mesh preview surface required before WR-029 can honestly claim model/mesh material pixels. | Basic | Current candidate: prove the source-backed Mesh Preview material-consuming pass with changed visible model/mesh pixels, selected material-region provenance, and refreshed WR-028 SDF non-regression evidence. | WR-030 is the current implementation row because the code-backed WR-029 Phase 1-3 material assignment path still lacks the visible model/mesh pixel proof required before WR-029 can close. |
| WR-031 | Workbench Clean-Break Governance And ADR | Platform foundation | current_candidate | not_applicable | L1 | Current governance gate | 4 | 3 | 4 | 5 | 4 | 3 | 0.8 | 4.5 | Accept the breaking Workbench decision before deleting compatibility paths. | Basic | Accepted ADR 0012 and roadmap/production records that explicitly approve dropping legacy Workbench persisted layout support. | Run architecture governance and accept the breaking Workbench clean-break ADR before code removes legacy profile or persistence paths. |
| WR-032 | Typed Suite, Surface, Profile, And Provider Handles | Platform foundation | ready_next | not_applicable | L1 | After WR-031 clean-break ADR | 4 | 3 | 4 | 5 | 4 | 5 | 0.8 | 2.7 | Add typed composition handles before removing old enum authority. | Basic | SuiteRef, SurfaceRef, ProfileRef, ProviderBundle, WorkbenchCompositionBuilder, and ToolSuiteProfileDefinition with exact stable-key tests. | Tool suites export typed handles; raw strings stay localized to declarations. |
| WR-033 | Remove Legacy Tool Surface Identity | Platform foundation | ready_next | not_applicable | L1 | After WR-032 typed handles | 4 | 3 | 5 | 5 | 5 | 8 | 0.5 | 1.2 | Delete ToolSurfaceKind as Workbench authority instead of preserving compatibility. | Basic | No Workbench code references ToolSurfaceKind, stable-key reverse mapping helpers, or legacy material/texture/diagnostics profile constructors. | Remove ToolSurfaceKind as Workbench identity and do not introduce a replacement compatibility enum. |
| WR-034 | Registry-Backed Workspace Profiles | Platform foundation | ready_next | not_applicable | L1 | After WR-033 legacy identity removal | 4 | 3 | 4 | 5 | 4 | 5 | 0.5 | 1.7 | Build workspace profiles only from installed suite/profile definitions. | Basic | Central legacy default profile authority deleted and replaced by Workbench composition data. | Installed suites and profile definitions own default workspace composition. |
| WR-035 | Clean Persistence Format | Platform foundation | ready_next | not_applicable | L1 | After WR-034 registry profiles | 4 | 3 | 5 | 5 | 5 | 5 | 0.5 | 1.9 | Replace legacy workspace persistence with stable-key-only schema. | Basic | Old persisted layouts fail with unsupported-schema diagnostics and new stable-key-only layouts round trip. | Old workspace files are unsupported, not migrated. |
| WR-036 | Material Lab Clean Migration Proof | Product workflow | ready_next | not_applicable | L2 | After WR-035 clean persistence | 4 | 3 | 4 | 5 | 4 | 5 | 0.5 | 1.7 | Prove Material Lab mounts through the clean Workbench model in both host modes. | Basic | Material Lab full-editor and standalone Workbench tests resolve without legacy kind metadata. | Material Lab is the proof surface for the clean registry foundation. |
| WR-037 | Host Capability Policy | Platform policy | ready_next | not_applicable | L2 | After WR-036 clean foundation proof | 4 | 3 | 4 | 5 | 4 | 5 | 0.5 | 1.7 | Add host allow/deny policy for command, product, and resource capabilities. | Basic | Constrained host denies a command capability before app/domain mutation. | Providers propose; hosts allow or deny; domains still validate semantics. |
| WR-038 | Product And Service Capability Declarations | Platform policy | ready_next | not_applicable | L2 | After WR-037 host policy | 4 | 2 | 4 | 5 | 4 | 8 | 0.5 | 1.1 | Let suites declare product and service needs without moving semantics into editor_shell. | Performance | Product and service declaration design accepted enough for implementation without semantic ownership leakage. | Product/service declarations stay declarative; product semantics stay in owning domains. |
| WR-039 | Multi-Host Presets | Platform host | ready_next | not_applicable | L2 | After WR-037 host policy | 4 | 3 | 4 | 4 | 4 | 5 | 0.5 | 1.6 | Full editor, Material Lab, headless validation, and constrained hosts share one clean model. | Basic | Host presets differ only by suite/profile/provider bundle and capability policy. | Host presets are Workbench composition data, not app forks or legacy route branches. |
