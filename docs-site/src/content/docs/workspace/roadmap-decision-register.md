...
| WR-089 | Renderer Procedural Population Hardening Doctrine And Track Activation | Render planning | current_candidate | bounded_contract | L4 | Doctrine activation before hardening implementation | 4 | 2 | 3 | 4 | 3 | 2 | 0.8 | 5.6 | N/A | Performance | Complete doctrine and track activation with active hardening design, production track metadata, WR-090, WR-091, WR-092, WR-101 implementation contracts, WR-093 closeout contract, the separate behavior-authoring intake, and a bounded-contract closeout for WR-089. | Intake accepted as a doctrine-first hardening track. WR-089 must not absorb indirect draw, primitive dispatch, graph catch-up, procedural camera, behavior-authoring, or unbounded population implementation. |
| WR-090 | Indirect Draw Contract Hardening | Render hardening | ready_next | not_applicable | L4 | After hardening doctrine activation | 4 | 2 | 4 | 5 | 4 | 5 | 0.8 | 2.7 | N/A | Performance | Promote only after WR-089 closes. Implementation must prove typed indexed/non-indexed indirect draw validation, byte-offset bounds, and removal or redesign of ignored indirect CPU offsets. | Ready-next hardening slice after doctrine activation. Indirect draw validation is the blocker before generated primitive indirect args can be runtime-proven. |
| WR-091 | Reusable GPU Primitive Shader Dispatch | Render hardening | ready_next | not_applicable | L4 | After indirect draw hardening | 4 | 2 | 5 | 5 | 4 | 8 | 0.8 | 1.8 | N/A | Performance | Promote only after WR-090 closes. Implementation must prove renderer-owned primitive kernels, hierarchical multi-block prefix scan, typed diagnostics, and non-boids runtime execution. | Ready-next primitive dispatch slice after indirect draw validation is fail-closed. |
| WR-092 | Fixed Step Graph Catch Up Scheduling | Render hardening | ready_next | not_applicable | L4 | After primitive shader dispatch | 4 | 2 | 4 | 4 | 4 | 5 | 0.8 | 2.6 | N/A | Performance | Promote only after WR-091 closes. Implementation must prove graph-level bounded repeated pass execution from runtime fixed-time resources, deterministic submitted substeps, iteration-scoped uniform projection, input/redraw-rate invariance, and resource sequencing across substeps. | Ready-next graph scheduling slice after primitive dispatch is runtime-proven. |
| WR-101 | Procedural Camera And View Projection | Render hardening | ready_next | not_applicable | L4 | After graph catch-up scheduling | 4 | 2 | 4 | 4 | 4 | 5 | 0.8 | 2.6 | N/A | Performance | Promote only after WR-092 closes. Implementation must prove reusable procedural 2D camera projection, fill-viewport aspect correctness, equal projected world x/y scale, and boids resize evidence without moving camera truth into PreparedViewFrame. | Ready-next procedural camera slice after graph catch-up scheduling makes simulation timing independent from redraw/input cadence. |
| WR-093 | Procedural Population Hardening Evidence Benchmarks Docs And Closeout | Render evidence | ready_next | not_applicable | L4 | After procedural camera projection | 4 | 2 | 3 | 4 | 3 | 3 | 0.8 | 3.7 | N/A | Performance | Promote only after WR-101 closes. Close the hardening track only when runtime evidence proves indirect validation, primitive shader dispatch, graph catch-up scheduling, procedural camera projection, benchmarks, and docs. | Ready-next evidence and closeout slice for PT-RENDER-PROCEDURAL-POPULATION-HARDENING. |
| WR-100 | UI Lab perfectionist governance and no-gap audit doctrine | Product planning | current_candidate | not_applicable | L4 | Current-candidate governance contract | 3 | 3 | 2 | 3 | 2 | 5 | 0.5 | 1.0 | N/A | Basic | Complete PM-UI-LAB-PERF-001 with accepted audit doctrine, code-truth matrix, evidence matrix, hard blockers, and disjoint follow-on WR candidates. | WR-100 is the current governance/design candidate for PT-UI-LAB-PERFECTION; it may update planning docs and audit doctrine only. |
