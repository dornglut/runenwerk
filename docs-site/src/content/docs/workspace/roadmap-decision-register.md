...
| WR-186 | Editor Docking And Cross-Window Composition Runtime | Editor UI | current_candidate | not_applicable | L4 | Adaptive headless proposal contracts runtime-proven | 5 | 2 | 4 | 5 | 5 | 13 | 0.8 | 1.2 | N/A | Must-be | A governed WR-186 implementation contract that maps every Region Compass state, app-owned policy decision, native-window lifecycle edge, and ordered core transaction. | Region Compass is selected; WR-185 is runtime-proven with typed five-zone intent and host-owned transaction materialization. |
