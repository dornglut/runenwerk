...
| PT-EDITOR-UX | Editor Product UX Lab And Surface Perfection | completed | editor | perfectionist_verified | Make the editor product UI certifiable through a native UX Lab, all-surface readiness policy, reusable design-system layers, graph/node-editor productization, standalone UI Designer workbench proof, and local-native no-gap evidence while preserving future game UI target-profile compatibility. | PT-EDITOR-UX is a new product UX track and does not reopen PT-UI-LAB or implement PT-GAME-RUNTIME-UI.<br>Generic UI truth remains in domain/ui, editor product semantics remain in domain/editor, and native evidence execution remains in apps/runenwerk_editor.<br>Every registered visible user-facing editor surface and explicit diagnostic/fallback surface is certified, fallback-only, diagnostic, or hidden until productized.<br>Every certified primitive widget, product widget pattern, registered surface, and host scenario has UX Lab coverage, visible-widget scan evidence, and state matrix evidence.<br>Final certification requires local-native screenshots where supported, accessibility and interaction reports, performance evidence, and zero hard-budget UI gaps.<br>Future game-runtime UI readiness is proven only through target-profile compatibility seams and never through editor-shell dependency. |
| PT-UI-DESIGNER-WORKBENCH | UI Designer Workbench Productization | completed | editor | runtime_proven | Deliver the real standalone and embedded UI Designer workbench described by the accepted product design while preserving generic UI truth in domain/ui, editor adapters in domain/editor, app execution and evidence in apps/runenwerk_editor, and game-runtime UI implementation downstream of a compatibility seam. | The track does not reopen PT-EDITOR-UX; archived WR-114 remains bounded evidence for an earlier workbench route and evidence slice.<br>Product implementation follows the accepted UI Designer Workbench Product Design and completes the V1 package, catalog, canvas, inspector, operation, scenario, evidence, and performance contracts before runtime_proven closeout.<br>Generic UI definition and token truth remain in domain/ui, editor/workbench adapters remain in domain/editor, and app-hosted execution plus evidence remain in apps/runenwerk_editor.<br>Game-runtime compatibility is proven only as a target-profile seam; runtime HUD behavior remains owned by PT-GAME-RUNTIME-UI or later game UI tracks.<br>Product readiness claims require code truth, source-versioned evidence, diagnostics, performance baselines, docs, examples, and roadmap/production validation. |
| PT-UI-DESIGNER-WB-V1-CLOSURE | UI Designer Workbench V1 Product Closure | completed | editor | runtime_proven | Repair the completed UI Designer Workbench productization overclaim by closing the accepted V1 product workflows with code-truth evidence, while preserving generic UI truth in domain/ui, editor adapters in domain/editor, app-hosted execution in apps/runenwerk_editor, and game-runtime HUD implementation in PT-GAME-RUNTIME-UI. | The track does not reopen PT-UI-DESIGNER-WORKBENCH implementation history; it records a governed closure path for accepted V1 workflow gaps discovered after the completed closeout.<br>The accepted UI Designer Workbench Product Design is the product contract for package/session source truth, recipe catalog insertion, hierarchy, canvas, inspector, operations, scenario matrix, evidence, and performance closure.<br>Canonical UI IR, recipes, bindings, fixtures, persistence, activation, and readiness contracts remain owned by domain/ui and domain/ui/ui_theme; editor/workbench adapters remain in domain/editor.<br>apps/runenwerk_editor owns concrete session state, preview orchestration, runtime launch, evidence capture, and user-facing workbench behavior without becoming generic UI source truth.<br>game.runtime compatibility is proven as target-profile descriptor, fixture, binding, intent, and evidence workflow only; concrete HUD runtime behavior remains downstream of PT-GAME-RUNTIME-UI.<br>Runtime-proven completion requires source-versioned code truth, normal author workflow evidence, typed diagnostics, deterministic diffs, performance baselines, docs, roadmap and production validation, and an explicit known-gap list. |
| PT-UI-DESIGNER-EVIDENCE-CORRECTION | UI Designer Workbench Runtime Evidence Correction | completed | editor | runtime_proven | Supersede the earlier UI Designer Workbench runtime-evidence overclaim with source-revisioned, type-separated, product-path evidence while preserving historical closeouts and keeping game-runtime UI execution owned by PT-GAME-RUNTIME-UI. | Descriptor compatibility and runtime product evidence are different app-owned contracts.<br>editor.workbench runtime readiness requires fresh source revision equality, content-addressed product artifacts, product-path measurement provenance, and focused tests.<br>game.runtime descriptor compatibility remains visible but cannot satisfy UI Designer Workbench runtime readiness.<br>Catalog rows expose structured metadata, filter state, disabled reasons, and insertion routes only for compatible rows.<br>Prior completed UI Designer Workbench rows are annotated as superseded rather than rewritten as if the historical closeouts never happened. |
...
| ID | Milestone | Kind | State | Roadmap links | Outcome |
|---|---|---|---|---|---|
| PM-EDITOR-UX-001 | Governance Truth Audit And Track Activation | design | completed | WR-111 | The editor product UX track exists with nine ordered milestones, an active design, completed WR-111 governance closeout evidence, and blocked follow-on rows for the native UX Lab through final no-gap certification. |
| PM-EDITOR-UX-002 | Native Editor UX Lab Scenarios And Evidence Harness | implementation | completed | WR-112 | Editor UX scenarios, args, controls, interactions, fixture matrices, local-native evidence manifests, and hard widget/surface gates are available for primitives, patterns, surfaces, and host scenarios. |
| PM-EDITOR-UX-003 | Layered Editor Design System Migration | implementation | completed | WR-113 | Editor UI styling and product patterns resolve through shared UI Designer token/recipe/state contracts while editor-specific semantics remain in domain/editor adapters. |
| PM-EDITOR-UX-004 | Standalone UI Designer Workbench | implementation | completed | WR-114 | Authors can use canvas, hierarchy, inspector, property panels, token/recipe/binding previews, scenario matrices, and readiness evidence in a standalone UI Designer workbench. |
| PM-EDITOR-UX-005 | Graph Canvas And Node Editor Productization | implementation | completed | WR-115 | Material graph UX is complete enough for product evidence, and SDF/procgen/gameplay/particle/animation graph surfaces are either productized, fallback-only/diagnostic, or hidden until productized. |
| PM-EDITOR-UX-006 | Shell And Product Pattern Polish | implementation | completed | WR-116 | Editor product patterns have reusable adapters, scenario coverage, state coverage, focus/keyboard coverage, overflow policy, and native evidence. |
| PM-EDITOR-UX-007 | All Registered Visible Surface Wave | implementation | completed | WR-117 | The editor has no visible misleading placeholders and every visible registered surface has readiness classification, scenario coverage, scenario evidence, and native proof where required. |
| PM-EDITOR-UX-008 | Game UI Readiness Seam | hardening | completed | WR-118 | Future game-runtime UI has compatible generic UI contracts and evidence descriptors while PT-GAME-RUNTIME-UI remains the owner of runtime HUD implementation. |
| PM-EDITOR-UX-009 | Final Local Native No Gap Certification | release | completed | WR-119 | PT-EDITOR-UX can claim perfectionist_verified only after local-native evidence and all hard zero-budget gates pass with empty known_quality_gaps. |

### PT-UI-DESIGNER-WORKBENCH - UI Designer Workbench Productization

| ID | Milestone | Kind | State | Roadmap links | Outcome |
|---|---|---|---|---|---|
| PM-UI-DESIGNER-WB-001 | Governance Code Truth And Track Activation | design | completed | WR-120 | PT-UI-DESIGNER-WORKBENCH has production metadata, active design gates, a drift report requirement, disjoint follow-on WR candidate scopes, validation gates, and explicit stop conditions without product code changes. |
| PM-UI-DESIGNER-WB-002 | V1 Package Document Session And Evidence Model | design | completed | N/A | The workbench has accepted source-truth and session boundaries, persistence shape, evidence packet freshness rules, diagnostics requirements, and first package fixtures. |
| PM-UI-DESIGNER-WB-003 | Standalone App Shell And Embedded Host Parity | implementation | completed | WR-122 | Authors can launch a UI Designer app directly, and the embedded editor workspace consumes the same package/session/evidence model. |
| PM-UI-DESIGNER-WB-004 | Catalog Hierarchy Canvas Inspector V1 | implementation | completed | WR-123 | The catalog, hierarchy, canvas, inspector, diagnostics, and diff/review surfaces support the shared component catalog and editor tool panel workflows. |
| PM-UI-DESIGNER-WB-005 | Operation Diff Apply And Rollback | implementation | completed | WR-124 | V1 authoring gestures are reviewable, deterministic, source-mapped, undoable, and fail closed when they cannot be serialized. |
| PM-UI-DESIGNER-WB-006 | Scenario Evidence And Performance Baselines | hardening | completed | WR-125 | Product readiness claims are backed by source-versioned evidence and measured interaction/performance counters. |
| PM-UI-DESIGNER-WB-007 | Game Runtime Compatibility Seam | hardening | completed | WR-118 | Future game-runtime UI has compatible Designer package, recipe, fixture, view-model, intent, safe-area, input, localization, accessibility, diagnostics, and evidence descriptors. |
| PM-UI-DESIGNER-WB-008 | Runtime Proven Closeout And Handoff | release | completed | WR-126 | PT-UI-DESIGNER-WORKBENCH has runtime-proven closeout evidence, truthful known gaps, usage docs, examples, and downstream handoff notes for game-runtime UI. |

### PT-UI-DESIGNER-WB-V1-CLOSURE - UI Designer Workbench V1 Product Closure

| ID | Milestone | Kind | State | Roadmap links | Outcome |
|---|---|---|---|---|---|
| PM-UI-DESIGNER-WB-V1-CLOSURE-001 | Governance Drift Audit And Correction Contract | design | completed | WR-127 | The closure track has explicit drift findings, DDD ownership, dependency direction, stop conditions, disjoint write scopes, validation gates, and a first implementation-contract path. |
| PM-UI-DESIGNER-WB-V1-CLOSURE-002 | Package Session Source Truth Closure | implementation | completed | WR-128 | The standalone and embedded workbench edit one source-versioned package whose draft, applied, rollback, reload, diagnostics, and evidence state are reconstructable and cannot become provider/runtime cache truth. |
| PM-UI-DESIGNER-WB-V1-CLOSURE-003 | Recipe Catalog Insertion And Authoring Surface Closure | implementation | completed | WR-129 | Authors can search compatible recipes, inspect slots/tokens/states/accessibility requirements, insert compatible recipes into the active document, and see hierarchy, canvas, inspector, diagnostics, and diff update from one source version. |
| PM-UI-DESIGNER-WB-V1-CLOSURE-004 | Operation Diff Apply Rollback Parity Closure | implementation | completed | WR-130 | Every supported canvas, hierarchy, inspector, and catalog gesture routes through typed operation reports with deterministic diffs, fail-closed diagnostics, history, and recovery evidence. |
| PM-UI-DESIGNER-WB-V1-CLOSURE-005 | Scenario Matrix Game Runtime And Evidence Closure | hardening | completed | WR-131 | Product readiness claims are backed by target-profile-aware scenario evidence, typed diagnostics snapshots, artifact freshness policy, and measured frame build, canvas projection, catalog projection, diagnostics projection, and resize relayout counters. |
| PM-UI-DESIGNER-WB-V1-CLOSURE-006 | Runtime-Proven Product Closeout And Handoff | release | completed | WR-132 | UI Designer Workbench V1 product closure has runtime-proven evidence for normal author workflows, docs, examples, validation, completion-quality classification, and explicit downstream boundaries. |

### PT-UI-DESIGNER-EVIDENCE-CORRECTION - UI Designer Workbench Runtime Evidence Correction

| ID | Milestone | Kind | State | Roadmap links | Outcome |
|---|---|---|---|---|---|
| PM-UI-DESIGNER-EVIDENCE-CORRECTION-001 | Runtime Evidence Source Revision And Catalog Correction | implementation | completed | WR-133 | Runtime readiness now accepts only fresh editor.workbench runtime product evidence; game.runtime compatibility remains descriptor-only; catalog rows are filterable structured product rows; editor_shell no longer owns game HUD recipe vocabulary. |
