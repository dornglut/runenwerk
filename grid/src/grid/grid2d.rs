...
impl<L: Copy + Default> Grid2d<L> {
	...
	where F: FnMut(IVec2, &L) -> V, V: Copy + Default {
		...
		V: Copy + Default,
	{
		...
		for y in 0..=self.height {
			for x in 0..=self.width {
				...
				grid[y][x] = CornerTile { mask, logic, visual };
			}
		}
	...
	pub fn mesh_tiles(&self) -> Vec<Vec<Tile<MeshId>>> {
		...
		for y in 0..=self.height {
			for x in 0..=self.width {
				...
				grid[y][x] = Tile { mask, logic, visual };
			}
		}
