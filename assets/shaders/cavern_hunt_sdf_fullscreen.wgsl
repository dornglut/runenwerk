...
struct CavernFrameUniform {
    params : WorldParams,
    primitives : array<GeometryPrimitive, 384u>,
    agents : array<Agent, 96u>,
};
...
@group(0) @binding(0)
var<uniform> frame : CavernFrameUniform;
...
fn terrain_distance(p : vec3<f32>) -> f32 {
    ...
    for (var i = 0u; i < frame.params.primitive_count; i = i + 1u) {
        let primitive = frame.primitives[i];
    ...
    let roof_cut_y = frame.params.floor_rock_height.z;
...
fn scene_distance(p: vec3<f32>) -> f32 {
    ...
    for (var i = 0u; i < frame.params.agent_count; i = i + 1u) {
        let h = select(0.45, 0.18, frame.agents[i].kind == 5u || frame.agents[i].kind == 6u);
        let d = sd_sphere(
            p - vec3<f32>(frame.agents[i].pos.x, h + frame.agents[i].radius, frame.agents[i].pos.y),
            frame.agents[i].radius,
        );
        scene = min(scene, d);
    }
    return scene;
...
fn hunter_team_color(team: u32) -> vec3<f32> {
    let slot = team % 8u;
    switch slot {
        case 0u: { return vec3<f32>(0.30, 0.75, 0.98); }
        case 1u: { return vec3<f32>(0.98, 0.48, 0.28); }
        case 2u: { return vec3<f32>(0.90, 0.30, 0.95); }
        case 3u: { return vec3<f32>(0.30, 0.92, 0.45); }
        case 4u: { return vec3<f32>(0.98, 0.88, 0.28); }
        case 5u: { return vec3<f32>(0.96, 0.40, 0.70); }
        case 6u: { return vec3<f32>(0.50, 0.78, 0.99); }
...
fn legacy_material_color(p: vec3<f32>) -> vec3<f32> {
    ...
    for (var i = 0u; i < frame.params.agent_count; i = i + 1u) {
        ...
        if (d < best) {
            best = d;
            switch frame.agents[i].kind {
                case 0u: { color = hunter_team_color(frame.agents[i].team); }
                case 1u: { color = vec3<f32>(0.79, 0.34, 0.31); }
                case 2u: { color = vec3<f32>(0.77, 0.53, 0.22); }
                case 3u: { color = vec3<f32>(0.73, 0.30, 0.78); }
                case 4u: { color = vec3<f32>(0.97, 0.18, 0.27); }
                case 5u: { color = vec3<f32>(0.93, 0.77, 0.28); }
                case 6u: { color = vec3<f32>(0.20, 0.96, 0.58); }
                case 12u: { color = mix(hunter_team_color(frame.agents[i].team), vec3<f32>(0.88, 0.96, 0.78), 0.35); }
                case 13u: { color = mix(hunter_team_color(frame.agents[i].team), vec3<f32>(0.16, 0.16, 0.16), 0.65); }
                default: { color = vec3<f32>(0.85, 0.85, 0.85); }
            }
        }
    }
...
fn fs_main(input: VsOut) -> @location(0) vec4<f32> {
    let size = max(frame.params.screen_size, vec2<f32>(1.0, 1.0));
    let frag = input.uv * size;
    let ndc = vec2<f32>(
    ...
    let yaw = frame.params.camera_orbit.x;
    let pitch = frame.params.camera_orbit.y;
    ...
    let ro = frame.params.camera_target_time.xyz - forward * frame.params.camera_orbit.z;
    let tan_half_fov = tan(frame.params.camera_orbit.w * 0.5);
    ...
    if (hit) {
        ...
        let base = legacy_material_color(p);
        let ambient = base * vec3<f32>(0.10, 0.12, 0.14);
        let diffuse = base * (0.85 * ndotl * shadow);
