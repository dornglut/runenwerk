...
When a `.blend` file is present, the engine treats it as source-of-truth and
auto-exports a sibling `.glb` (same stem) using Blender CLI (`blender -b`).
The generated `.glb` is then imported into the runtime model pipeline.
