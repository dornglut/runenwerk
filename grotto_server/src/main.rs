...
use anyhow::Result;
use cavern_hunt::domain::{NetConfigHotReloadState, load_server_network_config_from_path};
use cavern_hunt::{CavernHuntPlugin, CavernHuntServerPlugin};
use engine::plugins::render::domain::ShaderRegistryResource;
use engine::plugins::{
    NetworkRuntimeHandle, NetworkServerPlugin, RenderPlugin, ScenePlugin, UiInputPlugin,
...
async fn main() -> Result<()> {
    ...
    let verifier = if config.use_axiom_verifier {
        ...
        if let Some(secret) = secret {
            ...
            eprintln!(
                "server config enables Axiom verifier but dedicated_server_shared_secret is missing; using local verifier"
            );
            None
        }
    } else {
...
fn apply_operator_runtime_overrides(
    config: &mut cavern_hunt::domain::ServerNetworkConfigAssetV1,
