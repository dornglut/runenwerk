...
use editor_shell::{
    ...
    WorkspaceProfileLayoutSource, WorkspaceProfileManifest, tool_surface_kind_for_stable_key,
    workspace_profile_ref_for_id,
};
...
fn profile_manifest(spec: &WorkspaceProfileSpec) -> WorkspaceProfileManifest {
    let layout = built_in_layout_definition(spec);
...
fn built_in_layout_definition(spec: &WorkspaceProfileSpec) -> EditorWorkspaceLayoutDefinition {
    ...
    match spec.default_layout_template {
        ...
        WorkspaceLayoutTemplate::ToolWorkspace => tool_workspace_layout_definition(
            ...
            spec.default_surface_keys
                .iter()
                .map(|key| {
                    let stable_key = ToolSurfaceStableKey::new(*key)
                        .expect("compiled-in profile stable key should be valid");
                    let kind = tool_surface_kind_for_stable_key(&stable_key)
                        .expect("compiled-in profile surface should retain legacy placement metadata");
                    LayoutSurface {
                        key: stable_key.as_str().to_owned(),
...
mod tests {
    ...
    fn built_in_profiles_use_declarative_layout_definitions() {
        for manifest in full_editor_profiles()
            .into_iter()
            .chain(material_lab_profiles())
            .chain(ui_designer_profiles())
            .chain(headless_validation_profiles())
        {
            assert!(
                matches!(
    ...
    fn material_tool_layout_contains_every_declared_default_surface_once() {
        let manifest = material_lab_profiles().remove(0);
    ...
    fn scene_and_editor_design_preserve_fixed_layout_surface_sets() {
        let scene = profile_manifest(&FULL_EDITOR_PROFILE_SPECS[0]);
        ...
        let design = profile_manifest(&FULL_EDITOR_PROFILE_SPECS[2]);
