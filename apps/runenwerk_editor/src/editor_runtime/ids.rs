...
impl EditorRuntimeIdRegistry {
	...
	pub fn unregister_entity(&mut self, editor_id: EntityId) -> Option<SceneEntitySnapshot> {
		self.entities.remove(&editor_id).map(|record| {
			SceneEntitySnapshot::new(editor_id, record.display_name, record.parent)
		})
	}
	...
	pub fn resolve_entity(&self, entity_id: EntityId) -> Option<ecs::Entity> {
		self.entities.get(&entity_id).map(|record| record.ecs_entity)
	}
	...
	pub fn has_children(&self, entity_id: EntityId) -> bool {
		self.entities.values().any(|record| record.parent == Some(entity_id))
	}
	...
	pub fn resolve_resource_rust_type_id(
		&self,
