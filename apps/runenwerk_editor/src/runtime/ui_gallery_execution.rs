...
pub(super) fn execute_gallery_story(
    ...
    let mut run = match runner.begin(request) {
        Ok(run) => run,
        Err(result) => {
            return finish_failed_begin(story, result.into_report(story.expected_outcome.clone()));
        }
    };
    ...
    let button_diagnostics = button_runtime_report
        ...
        .collect::<Vec<_>>();
    run.record(
        story_evidence::RUNTIME_VIEW.result(button_runtime_report.passed(), button_diagnostics),
    );
