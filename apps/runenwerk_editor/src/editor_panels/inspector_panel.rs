...
use crate::editor_panels::{InspectorWidgetField, build_widget_fields};
use crate::editor_runtime::{
    EditorInspectorUiState, InspectorFieldDraft, RunenwerkEditorRuntime,
    build_component_inspector_sections,
};
...
impl InspectorComponentItem {
    pub fn from_descriptor(descriptor: SceneComponentDescriptor, is_selected: bool) -> Self {
        Self {
        ...
        match runtime.primary_inspect_target() {
            Some(InspectTarget::Entity(entity)) => {
                ...
                let available_component_types = runtime
                    ...
                    .into_iter()
                    .map(
                        |(component_type, display_name)| InspectorAvailableComponentItem {
            ...
            Some(InspectTarget::Component {
                ...
                let sections =
                    match build_component_inspector_sections(runtime, entity, component_type) {
                        Ok(sections) => sections
                            .into_iter()
                            .enumerate()
                            .map(|(index, section)| {
                                let key =
                                    inspector_section_key(entity, component_type, index, &section);
                        ...
                        Err(error) => {
                            return InspectorPanelViewModel::Error {
                                message: format!(
                                    "failed to build component inspector sections: {error:?}"
                                ),
                            };
                        }
                    };
...
pub fn flatten_editable_fields(sections: &[InspectorSection]) -> Vec<InspectorEditableField> {
...
fn is_supported_edit_value(value: &editor_inspector::InspectorValue) -> bool {
    matches!(
        value,
...
fn inspector_section_key(
    ...
    format!(
        ...
        entity.0, component_type.0, index, section.display_name
    )
}
