use editor_core::EntityId;
use scene::Vec3Value;

use crate::editor_app::RunenwerkEditorApp;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TranslateAxis {
	X,
	Y,
	Z,
}

#[derive(Debug, Clone, PartialEq)]
pub enum ViewportToolCommand {
	BeginTranslateDrag { entity: EntityId },
	UpdateTranslateDrag { delta: Vec3Value },
	BeginTranslateAxisDrag { entity: EntityId, axis: TranslateAxis },
	UpdateTranslateAxisDrag { amount: f32 },
	CommitTranslateDrag,
	CancelTranslateDrag,
}

pub struct ViewportToolController;

impl ViewportToolController {
	/// File: apps/runenwerk_editor/src/editor_panels/viewport_tools.rs
	/// Method: dispatch
	pub fn dispatch(
		app: &mut RunenwerkEditorApp,
		command: ViewportToolCommand,
	) -> Result<(), &'static str> {
		match command {
			ViewportToolCommand::BeginTranslateDrag { entity } => {
				begin_translate_preview(app, entity)?;
			}
			ViewportToolCommand::UpdateTranslateDrag { delta } => {
				app.update_translation_preview(delta)?;
				app.dispatch_tool_action(editor_tools::ToolAction::UpdatePreview)?;
			}
			ViewportToolCommand::BeginTranslateAxisDrag { entity, axis } => {
				begin_translate_preview(app, entity)?;
				app.tool_runtime_state_mut().set_translate_axis(Some(axis))?;
			}
			ViewportToolCommand::UpdateTranslateAxisDrag { amount } => {
				let axis = app
					.tool_runtime_state()
					.translate_axis()
					.ok_or("no active translate axis")?;

				let delta = axis_delta(axis, amount);
				app.update_translation_preview(delta)?;
				app.dispatch_tool_action(editor_tools::ToolAction::UpdatePreview)?;
			}
			ViewportToolCommand::CommitTranslateDrag => {
				app.dispatch_tool_action(editor_tools::ToolAction::CommitPreview)?;
			}
			ViewportToolCommand::CancelTranslateDrag => {
				app.dispatch_tool_action(editor_tools::ToolAction::CancelPreview)?;
			}
		}

		Ok(())
	}
}

/// File: apps/runenwerk_editor/src/editor_panels/viewport_tools.rs
/// Method: begin_translate_preview
fn begin_translate_preview(
	app: &mut RunenwerkEditorApp,
	entity: EntityId,
) -> Result<(), &'static str> {
	app.dispatch_tool_action(editor_tools::ToolAction::SelectSingle(
		editor_core::SelectionTarget::Entity(entity),
	))?;
	app.dispatch_tool_action(editor_tools::ToolAction::BeginPreview)?;
	Ok(())
}

/// File: apps/runenwerk_editor/src/editor_panels/viewport_tools.rs
/// Method: axis_delta
fn axis_delta(axis: TranslateAxis, amount: f32) -> Vec3Value {
	match axis {
		TranslateAxis::X => Vec3Value::new(amount, 0.0, 0.0),
		TranslateAxis::Y => Vec3Value::new(0.0, amount, 0.0),
		TranslateAxis::Z => Vec3Value::new(0.0, 0.0, amount),
	}
}