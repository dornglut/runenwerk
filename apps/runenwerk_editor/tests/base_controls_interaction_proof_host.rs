...
use ui_input::{FocusInputFact, FocusTargetId, Key, KeyState, KeyboardEvent, Modifiers, PointerButton, PointerDelta, PointerEvent, PointerEventKind, TextInputEvent, UiInputEvent};
use ui_math::UiPoint;
...
fn base_controls_interaction_proof_host_processes_events() {
    ...
    host.apply_input_event("move_button", pointer_event(PointerEventKind::Move, 12.0, 12.0));
    host.apply_input_event("press_button", pointer_event(PointerEventKind::Down, 12.0, 12.0));
    host.apply_input_event("release_button", pointer_event(PointerEventKind::Up, 12.0, 12.0));
    ...
    host.apply_input_event("disabled_button", pointer_event(PointerEventKind::Down, 12.0, 132.0));
    host.apply_input_event("outside", pointer_event(PointerEventKind::Down, 260.0, 260.0));
...
fn pointer_event(kind: PointerEventKind, x: f32, y: f32) -> UiInputEvent {
    UiInputEvent::Pointer(PointerEvent::new(kind, UiPoint::new(x, y), PointerDelta::ZERO, Some(PointerButton::Primary), Modifiers::default(), 1))
}

fn key_event(key: Key) -> UiInputEvent {
    UiInputEvent::Keyboard(KeyboardEvent { key, state: KeyState::Pressed, modifiers: Modifiers::default() })
}

fn text_event(text: &str) -> UiInputEvent {
    UiInputEvent::Text(TextInputEvent { text: text.to_owned() })
}

fn assert_marker(proof: &ui_runtime::InteractionVisualProof, widget_id: WidgetId, state: InteractionVisibleState) {
    let control = proof.main_view.control(widget_id).expect("proof should contain control");
