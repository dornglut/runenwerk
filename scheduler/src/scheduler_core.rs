...
impl<C> Scheduler<C> {
	pub fn new(dag: DAG<C>) -> Self {
		Self { dag, execution_order: vec![], dirty: true }
	}

	pub fn run(&mut self, ctx: &mut C) -> Result<()> {
		if self.dirty {
			self.rebuild_execution_order().context("Failed to rebuild execution order")?;
		}
		...
		for node_id in &self.execution_order {
			if let Some(node) = self.dag.get_node_mut(*node_id) {
				let node_span = span!(Level::DEBUG, "node_run", node_id = ?node_id, node_name = %node.name);
				let _enter_node = node_span.enter();

				let start = Instant::now();
				(node.func)(ctx)
					.with_context(|| format!("Failed to run node {:?}", node_id))?;
				let duration = start.elapsed();

				if duration.as_millis() > 1 {
					info!("Node {:?} ({}) took {} ms", node_id, node.name, duration.as_millis());
				}
			}
		}
		Ok(())
	...
	pub fn rebuild_execution_order(&mut self) -> Result<()> {
		...
		fn visit<C>(
			...
			if !visiting.insert(node) {
				anyhow::bail!("Cycle detected involving node {:?} ({})", node, dag.get_node(node).map(|n| &n.name).unwrap_or(&"<unknown>".to_string()));
			}
