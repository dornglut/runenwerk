	...
	match value {
		...
		InspectorEditValue::Integer(next) => {
			...
			if let Some(slot) = current.downcast_mut::<i16>() {
				*slot = i16::try_from(*next).map_err(|_| InspectorEditError::IntegerOutOfRange {
					target_type: "i16",
					value: *next,
				})?;
				return Ok(());
			...
			if let Some(slot) = current.downcast_mut::<i32>() {
				*slot = i32::try_from(*next).map_err(|_| InspectorEditError::IntegerOutOfRange {
					target_type: "i32",
					value: *next,
				})?;
				return Ok(());
			...
			if let Some(slot) = current.downcast_mut::<u16>() {
				*slot = u16::try_from(*next).map_err(|_| InspectorEditError::IntegerOutOfRange {
					target_type: "u16",
					value: *next,
				})?;
				return Ok(());
			...
			if let Some(slot) = current.downcast_mut::<u32>() {
				*slot = u32::try_from(*next).map_err(|_| InspectorEditError::IntegerOutOfRange {
					target_type: "u32",
					value: *next,
				})?;
				return Ok(());
			...
			if let Some(slot) = current.downcast_mut::<u64>() {
				*slot = u64::try_from(*next).map_err(|_| InspectorEditError::IntegerOutOfRange {
					target_type: "u64",
					value: *next,
				})?;
				return Ok(());
...
mod tests {
	...
	fn updates_nested_component_float_field_by_path() {
		...
		let position = world.get::<Position>(entity).expect("component should exist");
	...
	fn updates_resource_float_field_by_path() {
		...
		let bridge =
			StaticEcsInspectorBridge::new().with_resource_type::<CameraSettings>(ResourceTypeId(20));
