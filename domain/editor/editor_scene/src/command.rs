...
use crate::{
	...
	EditResourceFieldCommand, RemoveComponentCommand, RenameEntityCommand,
	ReparentEntityCommand, SceneCommandContext, SceneCommandIntent,
};
...
enum SceneCommandKind {
	...
	Unsupported(SceneCommandIntent),
}
...
impl SceneEditorCommand {
	...
	pub fn from_intent(
		...
		let kind = match intent {
			SceneCommandIntent::CreateEntity { parent, display_name } => {
			...
			SceneCommandIntent::RemoveComponent {
				entity,
				component_type,
			} => SceneCommandKind::RemoveComponent(RemoveComponentCommand::new(entity, component_type)),
			...
			SceneCommandIntent::RenameEntity {
				entity,
				new_display_name,
			} => SceneCommandKind::RenameEntity(RenameEntityCommand::new(
				entity,
				new_display_name,
			)),
			unsupported => SceneCommandKind::Unsupported(unsupported),
		};
...
impl Command for SceneEditorCommand {
	...
	fn apply<'a>(
		...
		match &mut self.kind {
			...
			SceneCommandKind::Unsupported(_) => return Ok(CommandOutcome::NoOp),
		}
...
mod tests {
	...
	fn edit_component_field_command_round_trips() {
		let mut session = editor_core::EditorSession::new();
		let mut runtime = MockSceneRuntime::default();
		let entity = runtime.create_entity(None, "Player").expect("entity create should succeed");
