...
impl World {
	...
	pub fn mark_stateful_changed<T: StatefulComponent>(&mut self, entity: Entity) -> bool {
		...
		self.mark_component_modified_by_id(entity, TypeId::of::<T>(), T::component_name());
		self.archetype_registry.mark_component_stateful_changed_by_id(
			entity,
			TypeId::of::<T>(),
			self.change_tick,
			&self.entity_locations,
	...
	pub fn __insert_component<T: Component>(
		...
		let inserted = if matches!(kind, crate::world::change_tracking::ComponentChangeKind::Added) {
	...
	fn mark_component_type_changed_by_id(&mut self, type_id: TypeId) {
		...
		self.component_change_ticks.insert(type_id, self.change_tick);
	...
	pub(crate) fn record_component_change(
		...
		if matches!(kind, crate::world::change_tracking::ComponentChangeKind::Removed) {
