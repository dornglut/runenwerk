...
impl OverlayLayeringScript {
    pub fn new(replay_id: impl Into<String>) -> Self { Self { replay_id: replay_id.into(), steps: Vec::new() } }
    pub fn with_step(mut self, step: OverlayLayeringStep) -> Self { self.steps.push(step); self }
}
...
impl OverlayLayeringStep {
    pub fn new(step_id: impl Into<String>, sample: NormalizedInputSample) -> Self { Self { step_id: step_id.into(), sample, viewport_rect: None, invalidated_anchor: None } }
    pub fn with_viewport_rect(mut self, viewport_rect: UiRect) -> Self { self.viewport_rect = Some(viewport_rect); self }
    pub fn with_invalidated_anchor(mut self, anchor_id: impl Into<String>) -> Self { self.invalidated_anchor = Some(anchor_id.into()); self }
}
...
pub fn base_controls_overlay_layering_negative_scripts() -> Vec<OverlayLayeringScript> {
    vec![OverlayLayeringScript::new("base-controls.overlay-layering.disabled").with_step(pointer_step("step.suppress.disabled-anchor", PointerEventKind::Down, 40.0, 458.0))]
}
...
fn pointer_step(id: &str, kind: PointerEventKind, x: f32, y: f32) -> OverlayLayeringStep {
    OverlayLayeringStep::new(id, NormalizedInputSample::new(format!("sample.{id}")).with_fact(NormalizedInputFact::Pointer(PointerInputFact::new(kind, UiPoint::new(x, y)))))
}

fn key_step(id: &str, key: Key) -> OverlayLayeringStep {
    OverlayLayeringStep::new(id, NormalizedInputSample::new(format!("sample.{id}")).with_fact(NormalizedInputFact::Keyboard(KeyboardInputFact::new(key, KeyState::Pressed))))
}

fn focus_step(id: &str, widget_id: WidgetId) -> OverlayLayeringStep {
    OverlayLayeringStep::new(id, NormalizedInputSample::new(format!("sample.{id}")).with_fact(NormalizedInputFact::Focus(FocusInputFact::target(FocusTargetId(widget_id.0)))))
}
