...
use ui_runtime::{
    ...
    base_controls_overlay_layering_positive_script, replay_overlay_layering, OverlayLayeringScript,
    OverlayLayeringStep,
};
...
fn overlay_layering_report_proves_required_rows_and_boundaries() {
    ...
    let report = replay_overlay_layering(&fixture, &base_controls_overlay_layering_positive_script());
    ...
    let submenu = report.stack_entries.iter().find(|row| row.anchor_id == "anchor.action-prompt.submenu").expect("submenu stack entry");
    ...
    assert!(report.keyboard_navigation_evidence.iter().any(|row| row.product_commands_executed == 0));
...
fn overlay_layering_inside_active_overlay_does_not_dismiss() {
    ...
    let script = OverlayLayeringScript::new("overlay.inside-active-proof")
        ...
        .with_step(pointer_step("step.pointer-inside-submenu-overlay", 40.0, 180.0));
