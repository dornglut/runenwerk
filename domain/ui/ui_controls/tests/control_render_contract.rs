...
#[test]
fn control_render_bridge_exposes_read_only_inspection_facts() {
    let facts = label_render_descriptor().summary().inspection_facts();

    assert!(facts.iter().any(|fact| {
        fact.key == "required_primitive_families" && fact.value.contains("glyph-run")
    }));
    assert!(facts.iter().any(|fact| {
        fact.key == "expected_primitive_counts" && fact.value.contains("glyph-run:1")
    }));
    assert!(facts.iter().any(|fact| {
        fact.key == "render_evidence_ids"
            && fact
                .value
                .contains("runenwerk.ui.controls.label.evidence.render.contract")
    }));
    assert!(
        facts
            .iter()
            .any(|fact| { fact.key == "has_backend_render_behavior" && fact.value == "false" })
    );
}
