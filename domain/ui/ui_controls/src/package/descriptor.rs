...
use super::ids::{ControlFixtureId, ControlKindId, ControlPackageId, ControlPackageVersion, ControlStoryId, ControlTargetProfileRef};
use super::metadata::{ControlBudgetEvidenceRequirement, ControlCatalogMetadata, ControlCompatibilityFlags, ControlFixtureDescriptor, ControlMountEligibility, ControlPackageCategory, ControlRenderEvidenceRequirement, ControlRequirement, ControlRouteRequirement, ControlStoryDescriptor, ControlTag};
...
pub struct ControlPackageDescriptor {
    ...
    #[serde(default)] pub story_ids: Vec<ControlStoryId>,
    #[serde(default)] pub stories: Vec<ControlStoryDescriptor>,
    #[serde(default)] pub interaction_descriptors: Vec<ControlInteractionDescriptor>,
    #[serde(default)] pub overlay_descriptors: Vec<ControlOverlayDescriptor>,
    #[serde(default)] pub mount_eligibility: ControlMountEligibility,
    #[serde(default)] pub binding_requirements: Vec<ControlRequirement>,
    #[serde(default)] pub theme_token_requirements: Vec<ControlRequirement>,
    #[serde(default)] pub accessibility_requirements: Vec<ControlRequirement>,
...
impl ControlPackageDescriptor {
    pub fn new(package_id: ControlPackageId, version: ControlPackageVersion) -> Self {
        Self { package_id, version, display_name: String::new(), description: String::new(), category: ControlPackageCategory::default(), tags: Vec::new(), target_profiles: Vec::new(), compatibility: ControlCompatibilityFlags::descriptor_only(), catalog_metadata: ControlCatalogMetadata::default(), control_kinds: Vec::new(), property_schemas: Vec::new(), state_schemas: Vec::new(), event_payload_schemas: Vec::new(), required_capabilities: Vec::new(), route_requirements: Vec::new(), kernels: Vec::new(), kernel_ids: Vec::new(), fixture_ids: Vec::new(), fixtures: Vec::new(), diagnostics: Vec::new(), diagnostic_ids: Vec::new(), migrations: Vec::new(), migration_ids: Vec::new(), story_ids: Vec::new(), stories: Vec::new(), interaction_descriptors: Vec::new(), overlay_descriptors: Vec::new(), mount_eligibility: ControlMountEligibility::default(), binding_requirements: Vec::new(), theme_token_requirements: Vec::new(), accessibility_requirements: Vec::new(), render_evidence_requirements: Vec::new(), budget_evidence_requirements: Vec::new(), deprecation: ControlDeprecationStatus::Active }
    }
    ...
    pub fn with_module(mut self, module: ControlModuleDescriptor) -> Self {
        ...
        for schema in schemas { match schema.role { ControlSchemaRole::Properties => self.property_schemas.push(schema), ControlSchemaRole::State => self.state_schemas.push(schema), ControlSchemaRole::EventPayload => self.event_payload_schemas.push(schema) } }
