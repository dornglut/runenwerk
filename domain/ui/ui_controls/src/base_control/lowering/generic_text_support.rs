...
use crate::{
    ...
    ControlGenericTextSemanticRole, ControlKindId,
};
...
pub(crate) fn lower_generic_text_support(
    ...
    let descriptor = match def.preset() {
        ControlPreset::Label => ControlGenericTextDescriptor::new(kind_id)
            .with_role(ControlGenericTextRoleDescriptor::new(
                "label.primary",
                ControlGenericTextSemanticRole::Label,
            ).with_inline_spans()),
        ControlPreset::Button => ControlGenericTextDescriptor::new(kind_id)
            .with_role(ControlGenericTextRoleDescriptor::new(
                "label.primary",
                ControlGenericTextSemanticRole::Label,
            )),
