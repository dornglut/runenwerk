...
impl ControlInspectionDescriptor {
    pub fn from_control_kind(
        ...
        push_fact(&mut facts, ControlInspectionSection::Surface2D, "surface2d.supported", bool_string(entry.surface2d_supported));
        push_fact(&mut facts, ControlInspectionSection::Surface2D, "surface2d.input_modes", &entry.surface2d_input_modes.join(","));
        push_fact(&mut facts, ControlInspectionSection::Surface2D, "surface2d.layers", &entry.surface2d_layers.join(","));
        push_fact(&mut facts, ControlInspectionSection::Surface2D, "surface2d.budget_evidence", &entry.surface2d_budget_evidence.join(","));
        push_fact(&mut facts, ControlInspectionSection::Surface2D, "surface2d.accessibility_complete", bool_string(entry.surface2d_accessibility_complete));
        push_fact(&mut facts, ControlInspectionSection::Surface2D, "surface2d.interaction_complete", bool_string(entry.surface2d_interaction_complete));
        push_fact(&mut facts, ControlInspectionSection::Surface2D, "surface2d.graph_or_timeline_semantics", bool_string(entry.surface2d_graph_or_timeline_semantics));
        Self {
    ...
    pub fn with_interaction_summary(mut self, summary: &ControlInteractionSupportSummary) -> Self {
        for fact in summary.inspection_facts() {
            push_fact(&mut self.facts, ControlInspectionSection::Interaction, &fact.key, &fact.value);
        }
        self
    }
    pub fn with_layering_summary(mut self, summary: &ControlOverlaySupportSummary) -> Self {
        for fact in summary.inspection_facts() {
            push_fact(&mut self.facts, ControlInspectionSection::Layering, &fact.key, &fact.value);
        }
        self
    }
    pub fn with_editable_text_summary(mut self, summary: &ControlEditableTextSupportSummary) -> Self {
        for fact in summary.inspection_facts() {
            push_fact(&mut self.facts, ControlInspectionSection::TextEditing, &fact.key, &fact.value);
        }
        self
    }
    pub fn with_generic_text_summary(mut self, summary: &ControlGenericTextSupportSummary) -> Self {
        for fact in summary.inspection_facts() {
            push_fact(&mut self.facts, ControlInspectionSection::TextDisplay, &fact.key, &fact.value);
        }
        self
    }
    ...
    pub fn with_accessibility_summary(mut self, summary: &ControlAccessibilityCapabilitySummary) -> Self {
        for fact in summary.inspection_facts() {
            push_fact(&mut self.facts, ControlInspectionSection::Accessibility, &fact.key, &fact.value);
        }
        self
    }
...
fn push_fact(facts: &mut Vec<ControlInspectionFact>, section: ControlInspectionSection, key: &str, value: &str) {
    facts.push(ControlInspectionFact { section, key: key.to_owned(), value: value.to_owned() });
}
