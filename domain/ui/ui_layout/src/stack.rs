	...
	pub fn arrange(&self, bounds: UiRect, children: &[StackItem]) -> Vec<UiRect> {
		...
		let (leading_offset, distributed_gap) = match self.main_align {
			...
			MainAxisAlignment::SpaceEvenly => {
				...
				let extra = if slots > 0.0 { extra_major / slots } else { 0.0 };
				(extra, gap + extra)
			}
		};
		...
		for (index, child) in children.iter().enumerate() {
			...
			let rect = match self.axis {
				Axis::Vertical => UiRect::new(cross_pos, bounds.y + cursor_major, cross_size, major),
				Axis::Horizontal => UiRect::new(bounds.x + cursor_major, cross_pos, major, cross_size),
			};
