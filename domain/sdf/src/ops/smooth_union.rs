...
impl<A, B> SmoothUnion<A, B> {
    pub fn new(left: A, right: B, smoothness: f32) -> Result<Self, ValidationError> {
        ensure_non_negative(smoothness, "smooth union smoothness")?;
        Ok(Self {
            left,
            right,
            smoothness,
...
impl<A, B> SdfField3 for SmoothUnion<A, B>
where
    A: SdfField3,
    B: SdfField3,
{
    fn sample(&self, point: Vec3) -> Result<SdfSample, SampleError> {
        ...
        if self.smoothness == 0.0 {
            return SdfSample::from_parts(
                left.signed_value().min(right.signed_value()),
    ...
    fn bounds(&self) -> FieldBounds {
        self.left
            .bounds()
            .union(self.right.bounds())
            .expanded_scalar(self.smoothness * 0.25)
    }
