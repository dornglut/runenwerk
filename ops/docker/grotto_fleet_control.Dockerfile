...
RUN cargo build --release -p grotto_fleet_control
...
COPY --from=builder /app/target/release/grotto_fleet_control /usr/local/bin/grotto_fleet_control

CMD ["/usr/local/bin/grotto_fleet_control", "--config", "/app/config/fleet.ron"]
